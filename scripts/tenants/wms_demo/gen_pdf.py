...
H2 = ParagraphStyle("H2", fontName="Helvetica-Bold", fontSize=11, leading=14,
                    textColor=colors.white, backColor=colors.HexColor("#1f2937"),
                    borderPadding=(4, 5, 4, 5), spaceBefore=4, spaceAfter=4)
NOTE = ParagraphStyle("NOTE", fontName="Helvetica", fontSize=8, leading=10.5,
                      textColor=colors.HexColor("#444444"))
LBL = ParagraphStyle("LBL", fontName="Helvetica-Bold", fontSize=8.5, leading=10.5)
SML = ParagraphStyle("SML", fontName="Helvetica", fontSize=7.2, leading=9,
                     textColor=colors.HexColor("#555555"))
CODE = ParagraphStyle("CODE", fontName="Courier", fontSize=7, leading=8.5,
                      textColor=colors.HexColor("#333333"))


def bc_code128(value, bar_width=0.40, height=13):
    return createBarcodeDrawing(
        "Code128", value=value, barHeight=height * mm, barWidth=bar_width * mm,
...
def bc_ean13(value, height=14):
    return createBarcodeDrawing(
        "EAN13", value=value, barHeight=height * mm, barWidth=0.36 * mm,
        humanReadable=True, fontSize=6.5, quiet=True,
    )
...
story.append(Paragraph(
    ...
    "(<font face='Courier' size='7'>/hht/wms/scan/resolve</font>).", SUB))
...
story.append(Paragraph(
    ...
    "khusus untuk uji serial/IMEI di bagian 5.", NOTE))
story.append(Spacer(1, 2 * mm))
story.append(grid([
    cell(bc_code128("JDC/IN/00001", 0.45, 16), "JDC/IN/00001",
         "Ready &middot; 4 baris &middot; 168 unit &middot; lot-tracked<br/>"
         "dipakai bagian 2, 3, 4 dan 7"),
    cell(bc_code128("JDC/IN/00012", 0.45, 16), "JDC/IN/00012",
         "Ready &middot; 1 baris &middot; 5 unit JD-TRK-01<br/>"
         "dipakai bagian 5 &mdash; serial / IMEI"),
], cols=2))
...
story.append(Paragraph(
    ...
    "Semua produk ini <b>lot-tracked</b>.", NOTE))
story.append(Spacer(1, 2 * mm))
story.append(grid([
    cell(bc_ean13(b), f"{sku} &nbsp;&mdash;&nbsp; {b}",
         f"{nm} &middot; demand <b>{q}</b> unit &middot; tracking: lot")
    for b, sku, nm, q in ON_RECEIPT], cols=2))
...
story.append(Paragraph(
    ...
    "bagian 2 &mdash; kalau tidak, jalur alias-nya putus.", NOTE))
story.append(Spacer(1, 2 * mm))
story.append(grid([
    cell(bc_code128("0" + b, 0.40, 12), f"0{b}", f"{sku} &middot; {nm}")
    for b, sku, nm, _q in ON_RECEIPT], cols=2))
...
story.append(Paragraph(
    ...
    "<b>expiration_date</b> harus ikut terisi dari AI 17.", NOTE))
story.append(Spacer(1, 2 * mm))
story.append(grid([
    cell(bc_code128(c, 0.26, 14), f"{sku} &middot; lot {lot}",
         f"kedaluwarsa {exp}", c)
    for c, sku, lot, exp in GS1], cols=2))
...
story.append(Paragraph(
    ...
    "hanya bentuk GS1 AI 21 yang bekerja.", NOTE))
story.append(Spacer(1, 2 * mm))
...
cells.append(cell(bc_code128("010899000000009921356938035643810", 0.28, 14),
                  "GS1 AI 01 + AI 21", "serial 356938035643810",
...
story.append(Paragraph(
    ...
    "di demo_wms sehingga scan-nya akan gagal.", NOTE))
story.append(Spacer(1, 2 * mm))
story.append(grid([
    cell(bc_code128(b, 0.42, 12), b, f"{path}<br/>{note}")
    for b, path, note in BINS], cols=2))
...
story.append(Paragraph(
    ...
    "menjawab <font face='Courier' size='7'>NOT_FOUND</font>.", NOTE))
story.append(Spacer(1, 2 * mm))
cells = [cell(bc_ean13(b), f"{sku} &nbsp;&mdash;&nbsp; {b}",
              f"{nm} &middot; <b>di luar</b> JDC/IN/00001")
         for b, sku, nm in OTHER]
cells.append(cell(bc_code128("9999999999999", 0.40, 12), "9999999999999",
                  "tidak terdaftar &mdash; harus NOT_FOUND"))
story.append(grid(cells, cols=2))
...
def footer(canvas, doc_):
    ...
    canvas.drawString(18 * mm, 8 * mm,
                      "demo_wms - JDC - lembar uji scan HHT receiving")
