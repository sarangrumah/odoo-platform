...
def xid_get(name):
    rec = env["ir.model.data"].search(
        [("module", "=", "wms_demo"), ("name", "=", name)], limit=1)
...
bins = env["stock.location"].search_count(
    [("usage", "=", "internal"), ("barcode", "like", "JDC-")])
p("Barcoded internal bins/zones: %d" % bins)

prods = env["product.template"].search([("barcode", "like", "899%")])
p("Products w/ EAN13: %d (lot-tracked: %d, expiry-enabled: %d)" % (
    len(prods),
...
if po:
    p("Inbound PO %s state=%s, receipts=%s" % (
        po.name, po.state,
        ", ".join("%s:%s" % (pk.name, pk.state) for pk in po.picking_ids)))

so = env["sale.order"].browse(xid_get("so_pick_demo"))
if so:
    p("Outbound SO %s state=%s, deliveries=%s" % (
        so.name, so.state,
        ", ".join("%s:%s" % (pk.name, pk.state) for pk in so.picking_ids)))

strat = env["custom.wms.putaway.strategy"].search([("warehouse_id", "=", wh.id)]) if wh else None
if strat:
    p("Putaway strategy: %s (%d rules, set=%s)" % (
        strat.name, len(strat.rule_ids), strat.rule_set))

plan = env["custom.cycle.count.plan"].search([("warehouse_id", "=", wh.id)]) if wh else None
if plan:
    sess = plan.session_ids
    p("Cycle count plan: %s (sessions=%d, lines=%d)" % (
        plan.name, len(sess), sum(sess.mapped("line_count"))))
...
if b2b:
    p("Bin-to-bin internal transfer: %s (%s -> %s) state=%s" % (
        b2b.name, b2b.location_id.name, b2b.location_dest_id.name, b2b.state))

quants = env["stock.quant"].search_count(
