...
if env["ir.config_parameter"].sudo().get_param(MARKER):
    ...
    if not strat.rule_ids:
        Rule.create([
            {"name": "T1 Footwear -> HD racking", "strategy_id": strat.id,
             "tier": 1, "kind": "fixed_location", "target_location_id": zone_hd,
             "product_domain": "[('categ_id', 'child_of', %d)]" % cat_foot},
            {"name": "T2 A-class -> NIKE pick", "strategy_id": strat.id,
             "tier": 2, "kind": "by_abc_velocity", "abc_class": "A",
             "target_location_id": sec_nike},
            {"name": "T3 Volume fit in PICK", "strategy_id": strat.id,
             "tier": 3, "kind": "by_volume",
             "target_location_domain": "[('location_id', 'child_of', %d)]" % zone_pick},
            {"name": "T4 Next empty in PICK", "strategy_id": strat.id,
             "tier": 4, "kind": "nearest_empty",
             "target_location_domain": "[('id', 'child_of', %d)]" % zone_pick},
            {"name": "T5 Next empty in HD", "strategy_id": strat.id,
             "tier": 5, "kind": "nearest_empty",
             "target_location_domain": "[('id', 'child_of', %d)]" % zone_hd},
            {"name": "T6 Any next empty bin", "strategy_id": strat.id,
             "tier": 6, "kind": "nearest_empty"},
        ])
    ...
    if not plan:
        ...
        env["ir.model.data"].create({
            "module": NS, "name": "cc_plan_pick", "model": "custom.cycle.count.plan",
            "res_id": plan.id, "noupdate": True})
        ...
        if sess:
            sess.action_start()
        log("cycle count plan + started session (%d lines)"
            % (sess.line_count if sess else 0))
    ...
    if hd_bin and nik_bin and peg and not have_hd:
        ...
        q = env["stock.quant"].with_context(inventory_mode=True).create({
            "product_id": variant.id, "location_id": hd_bin,
            "lot_id": lot.id, "inventory_quantity": 24,
        })
        ...
        env["ir.model.data"].create({
            "module": NS, "name": "bin2bin_picking", "model": "stock.picking",
            "res_id": picking.id, "noupdate": True})
        picking.action_confirm()
        picking.action_assign()

        # TO planning record (no materialize — engine.materialize is broken on /opt).
        env["custom.transfer.order"].create({
            "source_location_id": hd_bin, "target_location_id": nik_bin,
            "product_id": variant.id, "lot_id": lot.id,
            "planned_qty": 10, "state": "proposed",
        })
        log("bin-to-bin internal transfer %s staged: HD-A-01 -> NIK-02 (state %s)"
