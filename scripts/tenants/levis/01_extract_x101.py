"""Extract Levi's X101 Material Master to clean CSVs ready for Odoo import.

Tenant: PT Sinar Eka Selaras (SES) / Levi's. DB: ``levis``.

This is the era_busana_retailindo extractor re-pathed for Levi's. The Levi's
``X101_Material_Master.xlsx`` has the IDENTICAL 16-column layout as era_busana's,
so the extraction logic is unchanged. Only the source path + output namespace differ.

Decisions applied (same as era_busana):
- Dedup: keep most recent row per PROD SKU by PRICE EFFECTIVE FROM (descending).
- Encoding: source XLSX lost U+00AE (R-in-circle); we restore U+FFFD -> '®'.
- Variants: only (SIZE, INSEAM) combinations that appear in source.
- INSEAM '-' / None = no inseam (non-bottoms) -> single Size axis only.
- Currency: integer IDR, decimal=2 default Odoo IDR config (no scaling).

Run on the HOST (openpyxl is available on the host, not in the Odoo image):
    python scripts/tenants/levis/01_extract_x101.py

Outputs (CSV files in this directory, then copied into the container for 02_*):
  out_templates.csv          -> product.template
  out_attributes.csv         (Size + Inseam attribute values)
  out_template_attrlines.csv (tmpl_xid, attribute, value) -> product.template.attribute.line
  out_variants.csv           (one row per PROD SKU) -> product.product (default_code + barcode)
...
# repo root = scripts/tenants/levis -> ../../..
REPO_ROOT = os.path.abspath(os.path.join(HERE, "..", "..", ".."))
SRC = os.environ.get(
    "X101_SRC",
    os.path.join(REPO_ROOT, "docs", "levis", "X101_Material_Master.xlsx"),
)

# Encoding cleanup: source XLSX has U+FFFD where original was U+00AE (®)
REPL = {"�": "®"}
