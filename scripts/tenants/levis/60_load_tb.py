...
log(
    "company=%s currency=%s rounding=%s dry=%s opening_only=%s max_month=%s"
...
def post_move(ref, mdate, legs):
    ...
    move.action_post()
    log("posted %s : %d lines, total=%s" % (ref, len(line_ids), sum(l[2]["debit"] - l[2]["credit"] for l in line_ids)))
    return move
...
opening_legs = [
    (row["account"], row["description"] or ("Opening %s" % row["account"]), float(row["m1_beg"]))
    for row in tb
    if r(row["m1_beg"]) != 0
]
