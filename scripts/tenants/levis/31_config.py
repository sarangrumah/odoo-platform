...
for c in sample:
    cc = c.with_company(1)
    log(
        "CAT %-18s inc=%s exp=%s val=%s val_method=%s"
        % (
            c.name,
            cc.property_account_income_categ_id.code,
            cc.property_account_expense_categ_id.code,
            cc.property_stock_valuation_account_id.code,
            cc.property_valuation,
        )
    )
log(
    "tracking=lot templates: %d / total %d"
    % (env["product.template"].search_count([("tracking", "=", "lot")]), env["product.template"].search_count([]))
)
log("==== DONE ====")
