"""Import Levi's categories, attributes, templates, variants into the ``levis`` tenant DB.

Tenant: PT Sinar Eka Selaras (SES) / Levi's.

This is the era_busana_retailindo importer with the external-ID namespace switched
to ``levis`` and CSV_DIR switched to ``/tmp/levis``. The algorithm is unchanged.

Runs INSIDE odoo-mgmt container via:
    docker cp scripts/tenants/levis/out_*.csv odoo19-platform-odoo-mgmt:/tmp/levis/
    docker exec -i odoo19-platform-odoo-mgmt odoo shell -d levis --no-http < scripts/tenants/levis/02_import_to_odoo.py

Reads CSVs from /tmp/levis/ (copied in via `docker cp`).
Uses Odoo ORM with batched commits for performance on ~160k SKUs.

Strategy:
  1. product.category — 3-level tree (CATEGORY > CLASS > SUBCLASS).
  2. product.attribute — Size + Inseam, with all distinct values.
  3. product.template — with attribute_line_ids. Odoo auto-generates cartesian variants.
...
_logger = logging.getLogger("levis_import")
...
NS = "levis"  # ir.model.data module namespace for external IDs
...
for pass_prefix in ("cat_l1_", "cat_l2_", "cat_l3_"):
    for r in rows:
        ...
        ext = env["ir.model.data"].search(
            [("module", "=", NS), ("name", "=", r["xid"]), ("model", "=", "product.category")],
            limit=1,
        )
        if ext:
        ...
        env["ir.model.data"].create(
            {"module": NS, "name": r["xid"], "model": "product.category", "res_id": cat.id, "noupdate": True}
        )
...
# Pre-load existing
existing = env["ir.model.data"].search([("module", "=", NS), ("model", "=", "product.template")])
for ext in existing:
