...
log("accounts=%d opening=%d movement=%d closing=%d"
    % (len(accounts), len(opening), len(movement), len(closing)))
...
def balance_sheet(name, src, title, subtitle):
    ...
    ws.set_column(0, 0, 14); ws.set_column(1, 1, 46); ws.set_column(2, 4, 20)
    ws.freeze_panes(5, 0)
    r = 5
    td = tc = 0.0
    for aid, (code, aname) in accounts.items():
        ...
        td += dr; tc += cr
        r += 1
    ws.write(r, 0, "TOTAL", f_totl); ws.write(r, 1, "", f_totl)
    ws.write_number(r, 2, td, f_tot); ws.write_number(r, 3, tc, f_tot)
...
heads = ["No. Jurnal", "Tanggal", "Referensi (Toko)", "Jurnal", "Kode Akun", "Nama Akun",
         "Keterangan", "Debit", "Kredit"]
for i, h in enumerate(heads):
    ws.write(4, i, h, f_hdr)
ws.set_column(0, 0, 18); ws.set_column(1, 1, 12); ws.set_column(2, 2, 40); ws.set_column(3, 3, 10)
ws.set_column(4, 4, 14); ws.set_column(5, 5, 32); ws.set_column(6, 6, 34); ws.set_column(7, 8, 18)
ws.freeze_panes(5, 0)
r = 5
jd = jc = 0.0
...
heads = ["Kode", "Nama Akun", "Saldo Awal Debit", "Saldo Awal Kredit",
         "Mutasi Juni Debit", "Mutasi Juni Kredit", "Saldo Akhir Debit", "Saldo Akhir Kredit"]
for i, h in enumerate(heads):
    ws.write(4, i, h, f_hdr)
ws.set_column(0, 0, 14); ws.set_column(1, 1, 46); ws.set_column(2, 7, 20)
ws.freeze_panes(5, 0)
r = 5
tot = [0.0] * 6
for aid, (code, aname) in accounts.items():
...
ws0.set_column(0, 0, 52); ws0.set_column(1, 1, 26)
rows = [
