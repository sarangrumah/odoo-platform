...
class MockOracle:
    ...
    def add_member(self, member_id, member_no="M001", member_group="AREA1", balance=1000000.0):
        self.msg019t[member_id] = {
            "id": member_id, "member_no": member_no,
            "member_group": member_group, "deposit_balance": balance,
        }

    def add_msg016t(self, member_id, kode_voucher, trx_no, status="P",
                    sales_price=10000.0, msisdn="08123456789"):
        rid = self.next_msg016t_id
        self.next_msg016t_id += 1
        self.msg016t[rid] = {
            "id": rid, "trx_number_client": trx_no, "member_id": member_id,
            "kode_voucher": kode_voucher, "number_buyer": msisdn,
            "nominal_request": sales_price, "sales_price": sales_price, "fee_amount": 0,
            "is_ppob": "N", "status_ussd_2_provider": status, "request_date": None,
            "message_result_exec_ussd": "", "original_conversation_id": f"CONV_{rid}",
        }
        return rid
    ...
    def execute_sp(self, sp_name, params, out_specs):
        ...
        if "SellWithDenom" in sp_name:
            ...
            rid = self.add_msg016t(
                member_id=member_id, kode_voucher=params.get("kodevoucher", ""),
                trx_no=params.get("trxNumber", ""), status="P",
                sales_price=params.get("Nominal", 0), msisdn=params.get("mdn", ""),
            )
    ...
    def query(self, sql, params=None, fetch="all"):
        ...
        if "from msg016t" in sql_low and "where id =" in sql_low:
            ...
            tup = (row["id"], row["status_ussd_2_provider"],
                   row["message_result_exec_ussd"], row["sales_price"])
        ...
        if "from msg016t" in sql_low and "where id in" in sql_low:
            ...
            return [(r["id"], r["status_ussd_2_provider"], r["message_result_exec_ussd"])
                    for rid in ids for r in [self.msg016t.get(rid)] if r]

        if "from msg016t" in sql_low and "where id >" in sql_low:
            last_seen = params.get("last_seen", 0)
            limit = params.get("batch_size", 500)
            rows = sorted((r for r in self.msg016t.values() if r["id"] > last_seen),
                          key=lambda r: r["id"])[:limit]
            return [(
                r["id"], r["trx_number_client"], r["member_id"], r["kode_voucher"],
                r["number_buyer"], r["nominal_request"], r["sales_price"], r["fee_amount"],
                r["is_ppob"], r["status_ussd_2_provider"], r["request_date"],
                r["message_result_exec_ussd"], r["original_conversation_id"],
            ) for r in rows]

        if "from msg019t" in sql_low and "where id in" in sql_low:
            ids = [v for k, v in params.items() if k.startswith("id")]
            return [(self.msg019t[i]["id"], self.msg019t[i]["deposit_balance"])
                    for i in ids if i in self.msg019t]
...
class OracleBridgeCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        if not cls.product_class:
            cls.product_class = cls.env["custom.ppob.product.class"].create({
                "name": "Test Class", "code": "TEST"})

        cls.product = cls.env["custom.ppob.product"].create({
            "name": "Test TSEL10", "code": "TEST_TSEL10",
            "class_id": cls.product_class.id, "denom": 10000.0, "cost_price_default": 9500.0,
        })
        ...
        cls.provider_partner = cls.env["res.partner"].create({
            "name": "Test Provider Oracle", "x_custom_ppob_is_provider": True,
        })
    ...
    @staticmethod
    def _journal(cls):
        return cls.env["account.journal"].search([("type", "=", "general")], limit=1) \
            or cls.env["account.journal"].create({
                "name": "Test General", "type": "general", "code": "TST"})
