    ...
    def test_native_debit_blocked_on_oracle_mirror(self):
        ...
        with self.assertRaises(UserError) as ctx:
            wallet._atomic_debit(
                amount=100.0, reason="Native sale (should fail)",
                counterpart_account=counterpart, move_type="sale",
            )
    ...
    def test_native_credit_blocked_on_oracle_mirror(self):
        ...
        with self.assertRaises(UserError):
            wallet._atomic_credit(
                amount=100.0, reason="Native topup (should fail)",
                counterpart_account=counterpart, move_type="topup",
            )
