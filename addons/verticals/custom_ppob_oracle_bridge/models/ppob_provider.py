...
class PpobProvider(models.Model):
    ...
    oracle_sp_name = fields.Char(
        ...
        help="Stored procedure used by the Oracle bridge adapter's pay(). "
             "Schema prefix is added from the connection.",
    )
    ...
    def _check_bridge_mode_no_pending(self):
        ...
        for rec in self:
            ...
            if pending:
                raise UserError(_(
                    "Tidak bisa mengubah bridge_mode provider %s: ada %s "
