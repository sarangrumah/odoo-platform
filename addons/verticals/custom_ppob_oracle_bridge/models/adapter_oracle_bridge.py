...
class OracleBridgeAdapter(PPOBProviderAdapter):
    ...
    def pay(self, transaction):
        if oracledb is None:
            return AdapterResult(ok=False, error_code="oracledb_missing",
                                 error_message="oracledb library not installed on Odoo server")

        member_map = self._resolve_member(transaction)
        if not member_map:
            return AdapterResult(
                ok=False, error_code="member_not_mapped",
                error_message=("Mitra %s belum dimap ke Oracle MSG019T. "
                               "Tambahkan mapping di Oracle Bridge > Member Mapping.")
        ...
        if not sku_map or not sku_map.oracle_kode_voucher:
            return AdapterResult(
                ok=False, error_code="voucher_not_mapped",
                error_message=("Produk %s belum dimap ke KODE_VOUCHER Oracle. "
                               "Set field oracle_kode_voucher di SKU Map.")
    ...
    def status(self, provider_ref):
        ...
        try:
            ...
            return AdapterResult(ok=False, error_code="invalid_provider_ref",
                                 error_message="provider_ref is not a valid MSG016T.ID integer")

        connection = self._connection()
        try:
            row = connection.query(
                "SELECT id, status_ussd_2_provider, message_result_exec_ussd, sales_price "
        ...
        if not row:
            return AdapterResult(ok=False, error_code="msg016t_not_found",
                                 error_message="MSG016T row %s not found" % provider_ref)
    ...
    def topup(self, amount):
        raise NotImplementedError(
            "Topup via Oracle Bridge not implemented in phase 1. "
            "Use existing EVShop / VA H2H topup channels."
        )
