...
class PpobSlaTarget(models.Model):
    ...
    calibration_note = fields.Text(
        help="Where the numbers came from: which Oracle date range, which "
             "sample set, who agreed to them.",
    )
    ...
    p95_latency_ms_target = fields.Integer(
        ...
        default=3000,
        required=True,
        help="95th-percentile adapter round-trip budget. Measured against "
             "custom.ppob.transaction.provider_latency_ms.",
    )
        ...
        for target in self:
            ...
            if target.peak_factor < 1.0:
                raise ValidationError(_(
                    "Peak factor cannot be below 1.0 -- the peak rate is never "
