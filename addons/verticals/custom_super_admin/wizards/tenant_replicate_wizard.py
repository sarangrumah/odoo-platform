...
class TenantReplicateWizard(models.TransientModel):
    ...
    source_tenant_id = fields.Many2one(
        "tenant.registry", string="Source Tenant", required=True, ondelete="cascade",
    )
    source_env_type = fields.Selection(
        ENV_TYPES, string="Source Env", required=True, default="prod",
    )
    target_tenant_id = fields.Many2one(
        "tenant.registry", string="Target Tenant", required=True, ondelete="cascade",
        help="Usually the same slug as source — only the env_type differs.",
    )
    target_env_type = fields.Selection(
        ENV_TYPES, string="Target Env", required=True, default="staging",
    )
    ...
    def _check_distinct(self):
        for rec in self:
            if (
                rec.source_tenant_id == rec.target_tenant_id
                and rec.source_env_type == rec.target_env_type
            ):
                raise UserError(_(
                    "Source and target cannot be the exact same tenant + environment."
                ))

    def _resolve_backup(self):
        self.ensure_one()
        if self.latest_backup_only:
            backup = self.env["tenant.backup"].sudo().search(
                [
            ...
            if not backup:
                raise UserError(_(
                    "No successful backup found for source tenant %s."
                ) % self.source_tenant_id.slug)
            return backup
    ...
    def action_replicate(self):
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Restored backup %s into '%s' (%s env)."
                ) % (
