...
def sale_message(txn):
    ...
    if txn.state == "success":
        ...
        return (
            "Pengisian %(prod)s sebesar %(amt)s ke nomor %(mdn)s berhasil "
            "dengan no ref <%(ref)s>"
        ) % {
    ...
    if txn.state in ("failed", "timeout", "refunded"):
        ...
        return (
            "Pengisian %(prod)s sebesar %(amt)s ke nomor %(mdn)s GAGAL. %(msg)s"
        ) % {
            "prod": txn.product_id.name or txn.product_id.code or "produk",
            "amt": _amount(txn),
            "mdn": txn.msisdn or "",
...
def with_deposit(message, balance):
    ...
    return "%s Deposit Anda saat ini adalah %s." % (
        message or "", "{:,.2f}".format(balance or 0.0))
