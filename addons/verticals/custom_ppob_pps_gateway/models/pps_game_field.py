...
class PpsGameField(models.Model):
    ...
    product_id = fields.Many2one(
        comodel_name="custom.ppob.product", string="Product",
        required=True, ondelete="cascade", index=True,
    )
    key = fields.Char(
        required=True,
        help="Field key sent in the direct-topup 'field' object (e.g. userid, "
             "zoneid, server).",
    )
    ...
    _product_key_uniq = models.Constraint(
        "unique(product_id, key)", "Duplicate field key for this product.")
