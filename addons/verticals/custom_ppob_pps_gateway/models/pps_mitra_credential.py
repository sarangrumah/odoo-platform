...
class PpsMitraCredential(models.Model):
    ...
    pps_user = fields.Char(
        string="PPS User", required=True, index=True, copy=False,
        help="The 'user' login string POS sends on every PPS request.",
    )
    credential_ref = fields.Char(
        string="Password Key (ir.config_parameter)", required=True,
    ...
    _pps_user_uniq = models.Constraint(
        "unique(pps_user)", "PPS user login must be unique.")

    def _get_password(self):
        self.ensure_one()
        return self.env["ir.config_parameter"].sudo().get_param(
            self.credential_ref or "", "")
