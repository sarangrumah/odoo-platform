...
class _InquiryNameAdapter(PPOBProviderAdapter):
    def inquiry(self, transaction):
        return AdapterResult(ok=True, provider_ref="INQ-1",
                             raw={"nama": "BUDI SANTOSO", "customerName": "BUDI SANTOSO",
                                  "meterNumber": "4600123", "subscriberID": "0400000123",
                                  "electricityTariff": "R1/900VA"})

    def pay(self, transaction):
        return AdapterResult(ok=True, provider_ref="PAY-1", serial_token="SN-1",
...
class TestPpsGateway(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.mitra = cls.env["res.partner"].create({
            "name": "Mitra PPS", "x_custom_ppob_is_mitra": True,
            "x_custom_ppob_mitra_code": "PPSMTR1",
        })
        cls.cred = cls.env["custom.ppob.pps.mitra.credential"].create({
            "mitra_id": cls.mitra.id, "pps_user": "ERAUSER",
            "credential_ref": "pps.pwd.ERAUSER", "status": "active",
        })
        cls.vendor = cls.env["res.partner"].create({
            "name": "Biller PPS", "x_custom_ppob_is_provider": True})
        cls.product = cls.env["custom.ppob.product"].create({
            "code": "PPS_TSEL5", "name": "TSEL 5k", "class_id": cls.klass.id,
            "denom": 5000.0, "cost_price_default": 4900.0})

        # Funded mitra wallet (the PPS "deposit").
        cls.wallet = cls.env["custom.ppob.wallet"].create({
            "partner_id": cls.mitra.id, "class_id": cls.klass.id})
        cls.wallet._atomic_credit(amount=1_000_000.0, reason="seed",
                                  counterpart_account=cls.bank, move_type="topup")
        ...
        cls.game = cls.env["custom.ppob.product"].create({
            "code": "PPS_GAME1", "name": "Mobile Legends 100D", "class_id": cls.klass.id,
            "denom": 25000.0, "cost_price_default": 24000.0})
    ...
    def _make_provider(self, code, adapter):
        provider = self.env["custom.ppob.provider"].create({
            "code": code, "name": code, "partner_id": self.vendor.id,
            "settlement_mode": "prepaid_deposit", "bucket_mode": "bulky",
            "adapter_class": adapter})
        provider.action_ensure_buckets()
        provider.bucket_ids._atomic_credit(
            dpp_amount=1_000_000.0, tax_amount=0.0, gross_amount=1_000_000.0,
    ...
    def _map(self, provider, product):
        self.env["custom.ppob.provider.sku.map"].create({
            ...
            "provider_sku": f"SKU-{product.code}", "buy_price": product.cost_price_default,
            "priority": 100})
    ...
    def _post_form(self, path, params):
        return self.url_open(
            path, data=urlencode(params).encode(),
            headers={"Content-Type": "application/x-www-form-urlencoded",
                     "X-Odoo-Database": self.env.cr.dbname}, timeout=30)

    def _post_json(self, path, params):
        return self.url_open(
            path, data=json.dumps(params),
    ...
    def test_sell_success(self):
        notrx = "SELL-OK-1"
        p = {"user": "ERAUSER", "produk": self.product.code, "mdn": "081200000001",
             "notrx": notrx}
    ...
    def test_sell_bad_signature_rejected(self):
        p = {"user": "ERAUSER", "produk": self.product.code, "mdn": "0812", "notrx": "BADSIG",
             "signature": "deadbeef"}
    ...
    def test_sell_insufficient_deposit(self):
        ...
        mitra2 = self.env["res.partner"].create({
            "name": "Poor Mitra", "x_custom_ppob_is_mitra": True,
            "x_custom_ppob_mitra_code": "PPSMTR2"})
        self.env["custom.ppob.wallet"].create({
            "partner_id": mitra2.id, "class_id": self.klass.id})
        self.env["ir.config_parameter"].sudo().set_param("pps.pwd.POOR", "pw2")
        self.env["custom.ppob.pps.mitra.credential"].create({
            "mitra_id": mitra2.id, "pps_user": "POOR",
            "credential_ref": "pps.pwd.POOR", "status": "active"})
        notrx = "SELL-POOR-1"
    ...
    def test_checknocustomer_returns_name(self):
        notrx = "INQ-1"
        p = {"user": "ERAUSER", "product": self.product.code,
             "customer_no": "085700000001", "notrx": notrx}
    ...
    def test_inquiry_pln(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "custom_ppob_pps_gateway.pln_product_code", self.product.code)
    ...
    def test_direct_topup(self):
        notrx = "DTU-1"
        p = {"user": "ERAUSER", "product": self.game.code, "notrx": notrx,
             "field": {"userid": "9630001"}}
