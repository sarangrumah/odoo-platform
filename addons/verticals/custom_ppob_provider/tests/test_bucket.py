...
class TestProviderBucket(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_class = cls.env["custom.ppob.product.class"].search([], limit=1) \
            or cls.env["custom.ppob.product.class"].create({
                "code": "TST",
                "name": "Test Class",
            })
    ...
    def test_atomic_credit_bulky_split_journal(self):
        ...
        bucket._atomic_credit(
            dpp_amount=dpp, tax_amount=tax, gross_amount=gross,
            reason="Test topup",
    ...
    def test_atomic_debit_fixed_uses_correct_bucket(self):
        ...
        b5k._atomic_credit(
            dpp_amount=100_000.0, tax_amount=11_000.0, gross_amount=111_000.0,
            reason="topup 5k", counterpart_account=self.bank_account, move_type="topup",
        )
        b10k._atomic_credit(
            dpp_amount=200_000.0, tax_amount=22_000.0, gross_amount=222_000.0,
            reason="topup 10k", counterpart_account=self.bank_account, move_type="topup",
        )
        ...
        b10k._atomic_debit(
            amount=9_800.0, reason="sale TSEL10K",
            counterpart_account=self.cogs_account, move_type="usage",
        )
    ...
    def test_insufficient_balance_raises(self):
        ...
        with self.assertRaises(UserError):
            bucket._atomic_debit(
                amount=1.0, reason="nope", counterpart_account=self.cogs_account,
                move_type="usage",
            )
    ...
    def test_partial_unique_fixed_denom(self):
        ...
        self.env["custom.ppob.provider.bucket"].create({
            "provider_id": provider.id, "mode": "fixed_denom",
            "product_id": self.product_5k.id,
            "account_id": self.asset_account.id, "journal_id": self.journal.id,
        })
    ...
    def test_partial_unique_bulky(self):
        ...
        self.env["custom.ppob.provider.bucket"].create({
            "provider_id": provider.id, "mode": "bulky",
            "account_id": self.asset_account.id, "journal_id": self.journal.id,
        })
        with self.assertRaises(IntegrityError), self.cr.savepoint(), \
                mute_logger("odoo.sql_db"):
    ...
    def test_mode_switch_blocked_when_balance_nonzero(self):
        ...
        bucket = provider.bucket_ids
        bucket._atomic_credit(
            dpp_amount=10_000.0, tax_amount=0.0, gross_amount=10_000.0,
            reason="topup", counterpart_account=self.bank_account,
            move_type="topup",
        )
        with self.assertRaises(ValidationError):
