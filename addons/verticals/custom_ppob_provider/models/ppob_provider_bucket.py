...
class PpobProviderBucket(models.Model):
    ...
    low_water_mark = fields.Monetary(
        currency_field="currency_id",
        default=0.0,
        help="Warning threshold; transactions still allowed below this but an "
             "alert is raised to ops.",
    )
    ...
    inventory_warehouse_id = fields.Many2one(
        ...
        string="Warehouse",
        help="Warehouse used for stock pickings tied to this bucket. Defaults "
             "to the company default when blank.",
    )
    ...
    def _post_journal(self, dpp_amount, tax_amount, move_type, reason,
                      counterpart_account, partner=None):
        ...
        if move_type == "credit_bucket":
            if tax_amount and not self.provider_id.input_vat_account_id:
                raise UserError(_(
                    "Provider %s has no Input VAT account configured."
    ...
    def _atomic_credit(self, dpp_amount, reason, counterpart_account, *,
                       tax_amount=0.0, gross_amount=None, **extras):
        ...
        move = self._post_journal(
            dpp_amount, tax_amount, journal_type, reason, counterpart_account,
        )
        ...
        vals = {
            ...
            "gross_amount": (gross_amount if gross_amount is not None
                             else dpp_amount + (tax_amount if move_type == "topup" else 0.0)),
            "balance_after": new_balance,
            "ref": reason,
            "move_id": move.id,
            "state": "posted",
        }
        if extras.get("ppob_transaction_id") is not None \
    ...
    def _atomic_credit_from_move(self, move, *, dpp_amount, tax_amount=0.0,
                                 gross_amount=None, ref=None, move_type="topup"):
    ...
    def _atomic_debit(self, amount, reason, counterpart_account, **extras):
        ...
        if amount > fresh:
            raise UserError(_(
                "Insufficient bucket balance for %(b)s. "
                "Requested: %(req).2f. Available: %(avail).2f."
