...
class TestDigiflazzAdapter(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.wallet._atomic_credit(
            amount=5_000_000.0, reason="seed", counterpart_account=cls.bank, move_type="topup",
        )
        ...
        cls.provider.bucket_ids._atomic_credit(
            dpp_amount=5_000_000.0, tax_amount=0.0, gross_amount=5_000_000.0,
    ...
    def _reply(self, payload, status_code=200):
        ...
        return patch("odoo.addons.custom_ppob_biller_digiflazz.models.adapter_digiflazz."
                     "requests.post", side_effect=_fake_post), captured
    ...
    def test_sign_is_md5_of_username_apikey_refid(self):
        txn, captured = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Sukses", "rc": "00", "sn": "SN1", "price": 4900,
        }})
    ...
    def test_prepaid_topup_sends_no_commands_key(self):
        ...
        _txn, captured = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Sukses", "rc": "00", "price": 4900,
        }})
    ...
    def test_postpaid_pay_sends_pay_pasca(self):
        _txn, captured = self._dispatch_with_reply(
            ...
            product=self.postpaid_product, key="DF-POST",
        )
    ...
    def test_testing_flag_forwarded_only_when_set(self):
        ...
        _txn2, captured2 = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Sukses", "rc": "00", "price": 4900,
        }}, key="DF-2")
    ...
    def test_ref_id_is_stable_across_status_checks(self):
        ...
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Pending", "rc": "03", "price": 4900,
        }})
        original = txn.digiflazz_ref_id
        txn.dispatched_at = fields.Datetime.now() - timedelta(minutes=5)
        adapter = self.provider._get_adapter()
        patcher, captured = self._reply({"data": {
            "ref_id": original, "status": "Sukses", "rc": "00", "sn": "SN9", "price": 4900,
        }})
        with patcher:
    ...
    def test_retry_clone_gets_a_fresh_ref_id(self):
        ...
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Gagal", "rc": "01", "message": "gagal", "price": 4900,
        }})
    ...
    def test_sukses_settles_and_keeps_the_debit(self):
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Sukses", "rc": "00", "sn": "TOKEN-1", "price": 4900,
        }})
    ...
    def test_gagal_refunds(self):
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Gagal", "rc": "01", "message": "produk gangguan",
            "price": 4900,
        }})
        self.assertEqual(txn.state, "failed")
    ...
    def test_pending_does_not_refund(self):
        ...
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Pending", "rc": "03", "message": "sedang diproses",
            "price": 4900,
        }})
    ...
    def test_unknown_status_is_treated_as_pending_not_failure(self):
        ...
        txn, _ = self._dispatch_with_reply({"data": {
            "ref_id": "x", "status": "Antrian", "rc": "99", "price": 4900,
        }})
        self.assertEqual(txn.state, "in_progress")
        self.assertFalse(txn.wallet_refund_move_id)
    ...
    def test_transport_exception_refunds(self):
        ...
        with patch("odoo.addons.custom_ppob_biller_digiflazz.models.adapter_digiflazz."
                   "requests.post", side_effect=_requests.RequestException("boom")):
            txn._dispatch_one()
        self.assertEqual(txn.state, "failed")
        self.assertTrue(txn.wallet_refund_move_id)
    ...
    def test_status_proceeds_between_the_guards(self):
        ...
        adapter = self.provider._get_adapter()
        patcher, captured = self._reply({"data": {
            "ref_id": txn.digiflazz_ref_id, "status": "Sukses", "rc": "00",
            "sn": "SN-LATE", "price": 4900,
        }})
        with patcher:
            result = adapter.status(txn.digiflazz_ref_id)
        self.assertTrue(result.ok)
        self.assertEqual(result.raw["state"], "success",
                         "raw.state must speak the engine's vocabulary for the reaper")
        self.assertEqual(captured["body"]["ref_id"], txn.digiflazz_ref_id)
    ...
    def test_postpaid_status_uses_status_pasca(self):
        txn, _ = self._dispatch_with_reply(
            ...
            product=self.postpaid_product, key="DF-POST-2",
        )
        txn.dispatched_at = fields.Datetime.now() - timedelta(minutes=5)
        adapter = self.provider._get_adapter()
        patcher, captured = self._reply({"data": {
            "ref_id": txn.digiflazz_ref_id, "status": "Sukses", "rc": "00", "price": 100000,
        }})
        with patcher:
    ...
    def test_reaper_leaves_a_still_pending_transaction_alone(self):
        ...
        patcher, _captured = self._reply({"data": {
            "ref_id": txn.digiflazz_ref_id, "status": "Pending", "rc": "03", "price": 4900,
        }})
        with patcher:
    ...
    def test_reaper_refunds_a_transaction_that_failed_late(self):
        ...
        txn.dispatched_at = fields.Datetime.now() - timedelta(minutes=30)
        patcher, _captured = self._reply({"data": {
            "ref_id": txn.digiflazz_ref_id, "status": "Gagal", "rc": "01",
            "message": "timeout di operator", "price": 4900,
        }})
        with patcher:
            self.env["custom.ppob.transaction"]._cron_reap_stale_inprogress()
