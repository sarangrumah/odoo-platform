...
class TestLabelPrint(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product = cls.env["product.product"].create({
            "name": "Widget", "default_code": "WDG-1",
        })
    ...
    def test_queue_process_done(self):
        ...
        with patch.object(
                type(self.printer), "send_raw", return_value=True) as send:
            job.action_process()
    ...
    def test_cron_picks_only_queued(self):
        ...
        with patch.object(
                type(self.printer), "send_raw", return_value=True):
