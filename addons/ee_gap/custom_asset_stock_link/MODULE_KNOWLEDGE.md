...
**Lot identity is the asset code, and nothing else is written.** Registers built
from spreadsheets normally have a blank `serial_number`, and the asset code is
unique per unit, so the `stock.lot` is named after the code. This wizard used to
*also* back-fill a tenant register's `serial_number` with that code, so downstream
reports could join on the string. That is gone: it turned the register's Serial
Number column into a second copy of the asset code (3,590 of 3,590 on
`prd_arkaaim`), which is wrong the moment a register carries real printed serials
— and many units have none at all. The wizard now writes only `lot_id` and
`product_id`; consumers follow the FKs (`custom.fixed.asset.lot_id`,
`rental.asset.fixed_asset_id`) instead. See `custom_arka_aim_asset_register`.

In the search view the lot is labelled **Stock Serial/Lot** and its filter is
**No Stock Serial Yet**, to keep it distinct from a tenant `serial_number`.
