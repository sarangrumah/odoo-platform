...
class AssetStockMaterializeWizard(models.TransientModel):
    ...
    def _create_lots(self, assets, products):
        ...
        The asset code is the unique per-unit identifier and is what the lot is
        named after. A tenant module's ``serial_number`` holds the *physical*
        serial read off the unit, which many units simply do not have -- so it is
        never written from here. Back-filling it with the code produced a Serial
        Number column that just echoed the asset code; downstream joins now go
        through ``lot_id`` / ``fixed_asset_id`` instead of matching that string.
        """
        Lot = self.env["stock.lot"]
        lots = Lot
        for asset in assets:
            product = products[(asset.company_id.id, asset.name)]
            lot = Lot.search([("name", "=", asset.code), ("product_id", "=", product.id)], limit=1)
            if not lot:
                lot = Lot.create(
                    {
                        "name": asset.code,
            ...
            asset.write({"lot_id": lot.id, "product_id": product.id})
            lots |= lot
        return lots
