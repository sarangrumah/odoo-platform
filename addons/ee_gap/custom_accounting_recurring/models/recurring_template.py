...
class RecurringJournalTemplate(models.Model):
    ...
    line_ids = fields.One2many(
        ...
        "template_id", string="Lines", copy=True,
    )
    generated_move_ids = fields.One2many(
        "account.move", "custom_recurring_template_id",
        readonly=True, string="Generated Entries",
    )
        ...
        for tpl in self:
            ...
            if round(total_debit - total_credit, 2) != 0:
                raise ValidationError(_(
                    "Recurring template '%(name)s' is unbalanced: "
                    "debit=%(d).2f credit=%(c).2f.",
                    name=tpl.name, d=total_debit, c=total_credit,
                ))
    ...
    def _generate_one(self):
        ...
        if not self.line_ids:
            raise UserError(_(
                "Template '%(name)s' has no lines.", name=self.name,
            ))
        move_vals = {
