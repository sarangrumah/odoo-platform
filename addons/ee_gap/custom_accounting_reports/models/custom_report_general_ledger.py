...
class CustomReportGeneralLedger(models.AbstractModel):
    ...
    def _build_lines(self, filters):
        ...
        account_ids = sorted({r["account_id"] for r in rows} |
                             set(opening_by_account))
        move_ids = sorted({r["move_id"] for r in rows if r["move_id"]})
        partner_ids = sorted({
            r["partner_id"] for r in rows if r["partner_id"]
        })
        ...
        result = {"accounts": [], "grand_total_debit": 0.0,
        ...
        flat_lines.append({
            ...
            "closing": (
                result["grand_total_debit"]
                - result["grand_total_credit"]
            ),
        })
        return flat_lines
