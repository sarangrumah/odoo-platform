...
class CustomReportBalanceSheet(models.AbstractModel):
    ...
    def _build_lines(self, filters):
        ...
        def comp_section(type_codes, flip_sign):
            ...
            for row in comparison.values():
                if row["account_type"] not in type_codes:
                    continue
                signed = (
                    -row["balance"] if flip_sign else row["balance"]
                )
                subtotal += signed
            return subtotal
        ...
        asset_accounts, total_assets = self._section(
            "ASSETS", ASSET_TYPES, balances, flip_sign=False,
        )
        lines.append({
            "type": "section",
            "label": "Assets",
        ...
        liab_accounts, total_liab = self._section(
            "LIABILITIES", LIABILITY_TYPES, balances, flip_sign=True,
        )
        lines.append({
            "type": "section",
            "label": "Liabilities",
        ...
        eq_accounts, total_eq = self._section(
            "EQUITY", EQUITY_TYPES, balances, flip_sign=True,
        )
        lines.append({
            "type": "section",
            "label": "Equity",
