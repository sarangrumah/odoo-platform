...
- **Import PPN Masukan carries the faktur pajak pair.** `Tanggal Faktur Pajak` / `Nomor Faktur Pajak` sit between `Nomor Invoice` and `Nilai DPP`, read through `_opt` from `custom_coretax` (`x_custom_tanggal_faktur_pajak` / `x_custom_nsfp`) so the columns stay blank instead of crashing on a tenant without that module.
- **The Purchase Register is pulled per goods-receipt period, not per billing period.** `custom.report.purchase._gr_available()` (purchase + stock installed) gates a `date_basis` wizard selection that defaults to **Tanggal GR**: `_first_gr_dates()` reads the earliest *done* `stock.move` per `purchase.order.line` and only lines whose first receipt falls inside the window are pulled, whatever month their bill carries. Lines with no `purchase_line_id` — services, non-trade expenses, manual bills — have no receipt at all and keep their bill date as the basis, so switching basis never makes a cost vanish. The row still comes from the bill line (price, discount, tax live there); `Tgl GR` / `No. GR` / `Tgl Bill` are shown side by side, and `Item Code` is split out of `Item Name` (the old `Product` column carried `[CODE] NAME` in one cell). Month grouping follows the basis date.
