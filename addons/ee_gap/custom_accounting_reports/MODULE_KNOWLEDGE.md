...
- `custom.report.vat` — **Report VAT (Invoice PPN).** A *ledger* of the PPN accounts, not a per-faktur summary: one flat row per posted move line touching VAT In / VAT Out, with the account code and name repeated on every row, a running `Saldo Akhir` that opens from the balance carried before `date_from` and restarts per account, plus `No Faktur Pajak` and `Nama Pemasok`. Built for ARKA-AIM go-live item 2 ("ikutin Otomotif") to match a client reference pull; deliberately separate from `custom.report.faktur.pajak`, which answers the SPT-1111 question of one row per invoice with DPP beside PPN. VAT accounts are **derived from tax repartition lines**, minus any tax whose name starts with `PPH` — never hardcoded. A wizard account filter intersects that set, so it can narrow the pull but never add a non-VAT account. The three reference columns Odoo cannot supply verbatim (`No Bukti #`, `No. Trans #`, `No Referensi`) are mapped in `_transaction_no`, isolated there so Tax can change the mapping in one line.
