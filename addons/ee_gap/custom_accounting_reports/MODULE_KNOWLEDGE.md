...
- **Six stock registers live here, not in `custom_wms_reports`, and that is deliberate.** The client asked repeatedly to pull them from **Invoicing ▸ Reporting**, beside the Purchase Report. A menuitem here pointing at a WMS action would force this addon to depend on `custom_wms_reports` — dragging WMS, cycle counts, barcode and stock_account onto eight accounting databases with no warehouse. Building them here costs one SQL read per report and no dependency: every stock access is guarded by `custom.report.stock.mixin._stock_available()`, the same shape `custom.report.purchase._gr_available()` already uses.
- **🔴 POS sales never create a `stock.move` on this tenant.** 82,482 `pos.order.line` rows (91,008 units) against **zero** moves to a customer location. Anything claiming to show stock going "out" must read `pos.order.line` as a second source, or it shows receipts and returns and not one sale — and the running balance #64 asks for becomes meaningless. `_pos_lines` is that source.
- **🔴 On-hand is written by the X20 snapshot, not by a movement ledger**, so "opening ± movement" does not reconcile to `stock_quant`. The Summary carries the gap as its own **Adjustment** column and derives Beginning from Ending backwards, so `Beginning + In + Out + Adjustment = Ending` holds by construction and the gap is visible rather than hidden. `stock_valuation_layer` does not exist in this database and `stock_quant` is the position *now*.
- **These six read SQL, not browse records, and the difference is the whole feature.** The ORM version took 22.6s / 23.3s / 17.6s on `prd_levis_begbal`; the SQL version takes 3.0s / 2.3s / 1.3s. Sheet #59 is literally "berat untuk ditarik", so a browse loop over 46,500 moves, 82,482 POS lines and 45,000 quants is not an implementation detail. `_warehouse_by_location` materialises the location→warehouse map once instead of the correlated `parent_path` subquery the live Stock Summary runs per row, and `standard_price` is read as company-keyed jsonb rather than through `with_company` browse.
- **One QWeb template renders all six, from `_xlsx_columns()`.** Every other report here writes its `<th>` literally, which is why adding a column to them forces a `-u` across thirteen databases; here the PDF and the XLSX cannot disagree and a new column is Python only. `report_stock_generic` is reached from the `t-elif` chain by a tuple of codes, not six branches.
- **A grand total asserted as a constant is a test that only passes on an empty database.** `test_stock_reports` asserts shapes and invariants instead — the balance is per (warehouse, item), the summary arithmetic closes, the position ties to the summary's ending, a warehouse filter can only narrow.
