...
class TestCustomReports(TransactionCase):
    ...
    def _post_move(
        self, lines, dt=None, partner=None, ref=None,
    ):
    ...
    def test_trial_balance_zero_sum(self):
        ...
        ], dt=today, partner=self.partner_a, ref="m2")
        self._post_move([
            (self.acc_cash, 500.0, 0.0),
            (self.acc_recv, 0.0, 500.0),
        ], dt=today, partner=self.partner_a, ref="m3")
        self._post_move([
        ...
        ], dt=today, partner=self.partner_b, ref="m5")
        ...
        self.assertAlmostEqual(
            grand["movement_debit"], grand["movement_credit"], places=2,
            msg="Trial-balance debit total must equal credit total.",
        )
        self.assertGreater(
            grand["movement_debit"], 0.0,
    ...
    def test_aged_receivable_buckets(self):
        ...
        for i, (_overdue, amount, expected_bucket) in enumerate(cases):
            ...
            self.assertIsNotNone(
                row, f"Partner P{i} must appear in aged receivable.",
            )
            self.assertAlmostEqual(
                row[expected_bucket], amount, places=2,
                msg=(
                    f"Partner P{i}: expected {amount} in bucket "
                    f"{expected_bucket}, got {row[expected_bucket]}."
                ),
            )
    ...
    def test_general_ledger_partner_filter(self):
        ...
        ], dt=today, partner=self.partner_a, ref="A1")
        self._post_move([
            (self.acc_recv, 2000.0, 0.0),
            (self.acc_revenue, 0.0, 2000.0),
        ], dt=today, partner=self.partner_b, ref="B1")

        gl = self.env["custom.report.general.ledger"]
        lines = gl._build_lines(
            self._filters(partner_ids=[self.partner_a.id])
        )
        # Find Receivables account section.
        ar_section = next(
            (l for l in lines
             if l.get("type") == "account"
             and l.get("account_code") == "11100"),
            None,
        )
        self.assertIsNotNone(
            ar_section,
        ...
        for entry in ar_section["lines"]:
            self.assertEqual(
                entry.get("partner"), self.partner_a.name,
    ...
    def test_balance_sheet_equation(self):
        ...
        self._post_move([
            (self.acc_recv, 2000.0, 0.0),
            (self.acc_revenue, 0.0, 2000.0),
        ], dt=today, partner=self.partner_a, ref="sale")
        # Pays 500 expense.
        self._post_move([
        ...
        total_assets = next(
            l for l in lines
            if l.get("type") == "total"
            and l.get("label") == "Total Assets"
        )["signed_balance"]
        total_liab = next(
            l for l in lines
            if l.get("type") == "total"
            and l.get("label") == "Total Liabilities"
        )["signed_balance"]
        total_eq = next(
            l for l in lines
            if l.get("type") == "total"
            and l.get("label") == "Total Equity"
        )["signed_balance"]
        ...
        net_profit = next(
            l for l in pl_lines if l.get("type") == "grand_total"
        )["signed_balance"]

        self.assertAlmostEqual(
            total_assets - total_liab - total_eq, net_profit,
            places=2,
            msg=(
                "Assets - (Liab + Equity) must equal Net Profit "
                "before period close."
            ),
        )
    ...
    def test_tax_report_subtotals(self):
        ...
        lines = report._build_lines(
            self._filters(posted_only=False)
        )
        grand = next(
            (l for l in lines if l.get("type") == "grand_total"), None,
        )
        self.assertIsNotNone(grand, "Tax report must emit a grand_total.")
        category_subtotal = sum(
            l["tax_subtotal"]
            for l in lines
            if l.get("type") == "category"
        )
        self.assertAlmostEqual(
            category_subtotal, grand["tax_amount"], places=2,
