...
class TestAdapterHttp(PajakkuCommon):
    ...
    def test_submit_refused_when_disabled(self):
        ...
        with self.assertRaises(UserError):
            self.Adapter.submit_xml(b"<xml/>", config=self.config,
    ...
    def test_submit_happy_path_returns_uuid(self):
        token_resp = self._make_response(
            200, json_body={"access_token": "TKN", "expires_in": 3600}
        )
        submit_resp = self._make_response(
            200, json_body={"submission_uuid": "uuid-123", "message": "queued ok"}
        )
        with mock.patch.object(adapter_mod.requests, "post", return_value=token_resp), \
             mock.patch.object(adapter_mod.requests, "request", return_value=submit_resp):
            result = self.Adapter.submit_xml(
                b"<Faktur/>", config=self.config, transaction_type="efaktur_keluaran",
            )
    ...
    def test_submit_records_usage(self):
        ...
        submit_resp = self._make_response(
            200, json_body={"submission_uuid": "uuid-456"}
        )
        with mock.patch.object(adapter_mod.requests, "post", return_value=token_resp), \
             mock.patch.object(adapter_mod.requests, "request", return_value=submit_resp):
            self.Adapter.submit_xml(b"<x/>", config=self.config,
                                    transaction_type="efaktur_keluaran")
    ...
    def test_submit_error_increments_failure_streak(self):
        ...
        with mock.patch.object(adapter_mod.requests, "post", return_value=token_resp), \
             ...
             mock.patch.object(adapter_mod.time, "sleep"):  # don't actually wait
            with self.assertRaises(UserError):
                self.Adapter.submit_xml(b"<x/>", config=self.config,
                                        transaction_type="efaktur_keluaran")
        cid = self.config.company_id.id
    ...
    def test_oauth_failure_raises(self):
        ...
        with mock.patch.object(adapter_mod.requests, "post", return_value=bad_token_resp), \
    ...
    def test_query_nsfp_returns_nsfp_on_approved(self):
        ...
        # First call: submit
        submit_resp = self._make_response(
            200, json_body={"submission_uuid": "uuid-X"}
        )
        with mock.patch.object(adapter_mod.requests, "post", return_value=token_resp), \
             mock.patch.object(adapter_mod.requests, "request", return_value=submit_resp):
            r = self.Adapter.submit_xml(b"<x/>", config=self.config,
                                        transaction_type="efaktur_keluaran")
        # Second call: query
    ...
    def test_query_nsfp_returns_none_while_pending(self):
        ...
        pending_resp = self._make_response(
            200, json_body={"status": "processing"}
        )
        with mock.patch.object(adapter_mod.requests, "post", return_value=token_resp), \
