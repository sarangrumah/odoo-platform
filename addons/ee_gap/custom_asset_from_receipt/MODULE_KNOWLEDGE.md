...
- **A service is refused outright** — `_populate_lines` asks `product._can_be_fixed_asset()` *ahead of* the `_asset_conversion_mode_for` extension point, so a `type == 'service'` product is skipped whatever its master says and whatever a tenant override answers. The position is the point: ARKA-AIM's override returns `"quantity"` for every unconfigured product on a Non-Trade receipt, and services reach receipts at all now via `custom_service_receipt`.
