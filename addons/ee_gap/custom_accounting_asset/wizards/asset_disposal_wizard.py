...
class CustomFixedAssetDisposalWizard(models.TransientModel):
    ...
    def _compute_gain_loss(self):
        for wiz in self:
            wiz.gain_loss = (
                (wiz.disposal_value or 0.0)
                - (wiz.asset_id.net_book_value or 0.0)
            )
    ...
    def action_dispose(self):
        ...
        asset.message_post(body=_(
            "Asset disposed on %(date)s. Proceeds: %(value)s, gain/(loss): "
            "%(gain)s. %(note)s",
    ...
    def _create_disposal_move(self):
        ...
        if not asset.asset_account_id or not asset.depreciation_account_id:
            raise UserError(_(
                "Asset and accumulated depreciation accounts must be set on "
                "the asset to post a disposal entry."
            ))
