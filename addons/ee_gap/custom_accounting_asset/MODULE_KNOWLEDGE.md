...
- `custom.fixed.asset._post_due_depreciation(as_of=None)` — posts due unposted lines, **grouped into one `account.move` per (company, journal, expense account, accumulated account, date)** unless `_group_depreciation_moves()` is switched off.
