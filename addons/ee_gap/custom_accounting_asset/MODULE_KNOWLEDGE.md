...
- Create a `custom.fixed.asset` in `draft`; `code` auto-assigned from `ir.sequence("custom.fixed.asset")`. The group's five defaults (`useful_life_months`, `asset_account_id`, `depreciation_account_id`, `expense_account_id`, `journal_id` — the `_GROUP_DEFAULTS` tuple) **follow the group**: `_onchange_group_id` moves a value that is empty, still equal to the field default, or still equal to the *previous* group's default, and leaves a hand-typed value alone. `create()` applies the same five from `group_id` for keys the caller did not pass, so an import or an RPC call lands where the form would.
...
- **Depreciation dates are anchored on `posting_date`** (falls back to `acquisition_date` when empty) and shaped by `depreciation_date_mode`: `specific` (line 1 == posting date), `next_month` (default; line 1 == posting date + 1 month, the legacy behavior), `end_following_month` (last day of the month `seq` months after the anchor), `end_acquisition_month` (last day of the acquisition month itself for line 1 — the same arithmetic one ordinal earlier). `posting_date` defaults to `acquisition_date` on create. **`end_acquisition_month` ships switched off**: the field default comes from `_default_depreciation_date_mode()`, which reads `custom_accounting_asset.default_depreciation_date_mode` and falls back to `next_month`. No tenant has that parameter set — the running registers are already in the shape their accountants signed off on.
- Monthly cron `_cron_post_due_depreciation` (calls `_post_due_depreciation()`): walks all `state='running'` assets, posts every unposted line whose `date <= today` (DR expense / CR accumulated) and flips `line.posted=True` and `line.move_id`. Lines are **grouped into one `account.move` per (company, journal, expense account, accumulated account, line date, analytic distribution)** — a monthly run yields one document per date, not one per asset. `_group_depreciation_moves()` reads `custom_accounting_asset.group_depreciation_moves` (default `1`); set it to `0` on a tenant whose Accounting genuinely wants one entry per asset.
...
- **Folding historic per-unit assets into a pool** — `_merge_assets_into_pool(others)` sums acquisition, salvage, revaluation and surplus onto the survivor, sets `quantity`/`original_quantity` to the unit count, carries the absorbed accumulated depreciation in `opening_accumulated_depreciation`, drops the absorbed records' unposted lines, flags them `merged_into_id` + `cancelled`, and rebuilds the survivor's schedule. **Nothing is posted to the GL** — cost and accumulated depreciation are already there; only the subledger is rearranged, and the pooled NBV equals the sum of the NBVs that went in. `scripts/consolidate_pooled_assets.py` finds the candidates (strict signature match incl. the exact set of posted depreciation dates) and drives it, dry-run by default.
...
- `custom.fixed.asset.opening_accumulated_depreciation` (Monetary, readonly) — accumulated depreciation with no posted line behind it: an opening balance loaded at cutover, or depreciation absorbed from merged assets. **Added** to the posted total.
- `custom.fixed.asset.merged_into_id` / `merged_asset_ids` — an absorbed record points at the pool it joined; it stays `cancelled` with its posted history intact and is never depreciated again (the cron only walks `running`).
- `custom.fixed.asset.accumulated_depreciation` / `net_book_value` (Monetary, computed, non-stored) — `sum(posted lines) + opening_accumulated_depreciation - retired_accumulated_depreciation` and `acquisition + revaluation_value - accum`.
...
- `custom.fixed.asset._merge_assets_into_pool(others)` — absorb per-unit assets into this one; refuses across companies/currencies, on non-draft/running records, on already-merged records and on records carrying partial retirements.
...
- `custom.fixed.asset._post_due_depreciation(as_of=None)` — posts due unposted lines, **grouped into one `account.move` per (company, journal, expense account, accumulated account, date, analytic distribution)** unless `_group_depreciation_moves()` is switched off.
- `custom.fixed.asset._group_depreciation_moves()` — the grouping opt-out, read from `custom_accounting_asset.group_depreciation_moves`.
- `custom.fixed.asset._apply_group_defaults_to_vals(vals)` — cascades the group's five defaults into `create` vals for keys the caller omitted.
- `custom.fixed.asset._group_defaults_source()` — the group whose defaults currently sit on the record (`_origin.group_id`, or recovered from the account fingerprint on an unsaved form).
- `custom.fixed.asset._default_depreciation_date_mode()` — tenant default for `depreciation_date_mode`, from `custom_accounting_asset.default_depreciation_date_mode`.
...
- **The asset's analytic distribution reaches every entry it writes.** `custom.fixed.asset` inherits `analytic.mixin`, so it carries `analytic_distribution` (and gets the mixin's GIN index for free). `_stamp_analytic()` puts that distribution on **both** legs of a depreciation entry and on **every** leg of a disposal, partial disposal, revaluation and partial reversal — five journal writers in all. Sheet rows #24 and #57: Accounting reads the Operating Unit off the journal entry, so a distribution on only the P&L side would leave half the document unattributed. The stamp uses `setdefault`, so a caller with a more specific distribution for one leg keeps it, and an asset with no distribution posts exactly as before (ARKA-AIM runs 3,590 of those).
- **The analytic distribution is part of the depreciation grouping key.** Two assets in different Operating Units on the same date produce **two** entries, not one — a grouped entry carrying one store's analytic while covering another's would be false. Two assets in the *same* store still group into one document, because the distribution is added to the key rather than substituted for it. The key needs `_analytic_distribution_key()` because `analytic_distribution` is a Json dict and dicts are not hashable.
- **Depreciation moves are GROUPED, one per period date, not one per line** — a monthly run on ARKA-AIM's 3,300+ running assets produces a handful of documents, not thousands. The per-asset detail stays in this subledger. Consequences: reversing a line usually cannot reverse the whole move (see `depreciation_line._reverse_partial`), and anything that reads `line.move_id` expecting a private entry is wrong. `custom_accounting_asset.group_depreciation_moves = 0` restores one move per line.
...
- **Group defaults follow the group, but only where nobody overrode them** — `_onchange_group_id` moves a value that is empty, still the field default, or still the previous group's default, and keeps anything typed by hand. That protection is not cosmetic: 410 of ARKA-AIM's 3,590 assets deliberately run a useful life different from their group's (FA-PENDUKUNG 266/266, FA-KOMPONEN 144/1,824). It also matters the other way round — `_post_due_depreciation` reads the accounts off the asset **header**, so an account that fails to follow a group change is wrong in every future depreciation entry.
- **`create()` cascades the group defaults, `write()` does NOT** — a code-level `write({'group_id': ...})` still leaves the five fields where they were, by design: a mass re-grouping must not silently re-point the COA of assets that are already depreciating. Use the form, or write the fields explicitly.
- **The previous group is `_origin.group_id`, and on an unsaved form it is recovered from the accounts** — `_group_defaults_source()` matches all four account/journal defaults at once to work out which group last wrote them, so flipping the group twice before the first save still works. An asset whose accounts were edited by hand matches no group and is left alone.
- **A 0-based schedule is never rebuilt** — `_build_schedule` bails out when `min(sequence) == 0`. Sixty FA-OFFC assets in `prd_levis_begbal` were loaded with sequences 0..47 and their August line already posted; the ordinal arithmetic in `_depreciation_date_for` assumes 1-based, so a rebuild would issue a second line on a date that is already in the GL. Note `len(posted) + 1` is NOT a safe anchor for that cohort either — under its `end_following_month` mode it would skip a month.
