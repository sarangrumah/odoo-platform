...
- `custom.fixed.asset.is_quantity_asset` (Boolean, computed **stored**) — True when the asset holds more than one unit or has had units retired. Drives the pooled-only UI and the *Pooled (multi-unit)* filter.
