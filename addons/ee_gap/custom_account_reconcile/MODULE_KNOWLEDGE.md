...
- **Two different things hang, and sheet #61 is both.** `reconcile.overview.action_open_lines` opened an **unbounded** list — 63,240 lines for one GR/IR account on `prd_levis_begbal`, still 7,314 after the netting landed — and the existing `custom.account.reconcile.wizard` loads every selected line into `line_ids` as a `Command.set`, which is what actually freezes the browser. The action now carries `limit: 500`; `custom.account.reconcile.batch.wizard` holds **no lines at all** and works from a domain.
- **🔴 Scope on the batch wizard is mandatory, and it is not cosmetic.** Plain FIFO over this data settles to about three open lines, which would tie 7,314 lines into **one** `account.full.reconcile` — quadratic to create, and a nightmare the first time anyone resets one of those entries to draft. Grouping by partner and/or month keeps each reconciliation the size of a real business event.
- **A one-sided group is skipped, not forced.** An account with only debits left is an open position, not a reconciliation waiting to happen. The wizard counts those separately so the operator can see the difference.
- **The run is synchronous but bounded by `max_groups`.** A wizard that commits in a loop is how you get a half-finished ledger and no way to tell which half. Run it again to continue; the groups already closed are simply no longer open.
- **FIFO lives in `tools/fifo.py` as a plain function, on purpose.** `custom_accounting_reports` has the same logic as `custom.report.gl.open.items._fifo`, but it is a method on an AbstractModel and cannot be imported, and a dependency either way would force one module onto a database that does not want it (`custom_account_reconcile` depends on `account` alone). One small duplication, kept honest by tests.
