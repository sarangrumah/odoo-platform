...
class CustomAffiliateClick(models.Model):
    ...
    @api.model
    def _record_click(self, affiliate, link=None, landing_url=None, referrer=None,
                      session_key=None, ip=None, user_agent=None, dedup_seconds=60):
        ...
        if dedup_seconds and session_key:
            recent = self.sudo().search([
                ...
                ("create_date", ">=", fields.Datetime.subtract(
                    fields.Datetime.now(), seconds=dedup_seconds)),
            ], limit=1)
            if recent:
                return recent
