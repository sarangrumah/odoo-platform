...
class CustomAffiliatePayout(models.Model):
    _name = "custom.affiliate.payout"
    _description = "Affiliate Payout"
    _inherit = ["mail.thread"]
    _order = "create_date desc"

    name = fields.Char(
        required=True, copy=False, default=lambda self: _("New"), readonly=True
    )
    affiliate_id = fields.Many2one(
        "custom.affiliate", string="Affiliate", required=True, ondelete="restrict"
    )
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("paid", "Paid")], default="draft", required=True, tracking=True
    )
