...
class CustomQualityCapa(models.Model):
    ...
    signature_ids = fields.One2many(
        "custom.quality.signature", "capa_id", string="Sign-off Signatures",
    )
    ...
    def action_done(self):
        for rec in self:
            ...
            alert = rec.alert_id
            if alert and all(c.state in ("done", "canceled")
                             for c in alert.capa_ids):
