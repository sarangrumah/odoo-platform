...
class ToEngine(models.AbstractModel):
    ...
    def _eval_low_water_mark(self, rule):
        ...
        src_dom = [("location_id.usage", "=", "internal")] + rule._eval_domain(
            rule.source_location_domain
        )
        tgt_dom = [("location_id.usage", "=", "internal")] + rule._eval_domain(
            rule.target_location_domain
        )
