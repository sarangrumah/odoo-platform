...
VERTICAL_WRITABLE = {
    "name", "code", "legal_entity", "brand_group", "vertical_po_id", "ba_ids",
    "pic_partner_ids", "color", "sequence", "active",
}
STAGE_WRITABLE = {
    "name", "custom_code", "sequence", "custom_applies_to", "custom_sla_clock",
    "custom_is_hold", "custom_is_waiting_user", "custom_is_closed_stage",
    "custom_auto_close_days", "custom_require_reason", "fold",
}
RULE_WRITABLE = {
    "event", "recipient_kind", "role_group_id", "channel_wa", "channel_email",
    "channel_odoo", "sequence", "active",
}
...
def _audit(record, values):
    record._pdp_audit_write(
        "master_data_change", record.id, values,
...
class VaspmoAdminApi(http.Controller):
    ...
    # ------------------------------------------------------------- verticals
    @http.route("/vaspmo/api/admin/verticals", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    @_guard
    def verticals(self, **kw):
        records = request.env["custom.project.vertical"].with_context(
            active_test=False
        ).search([])
        return ok([{
            "id": v.id,
    ...
    @http.route("/vaspmo/api/admin/verticals/<int:vertical_id>", type="http",
                auth="jwt_vaspmo", methods=["POST", "PATCH"], csrf=False, save_session=False)
    @_guard
    def vertical_write(self, vertical_id, **kw):
        record = request.env["custom.project.vertical"].with_context(
            active_test=False
        ).browse(vertical_id)
    ...
    # ---------------------------------------------------------------- stages
    @http.route("/vaspmo/api/admin/stages", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    @_guard
    def stages(self, **kw):
    ...
    @http.route("/vaspmo/api/admin/stages/<int:stage_id>", type="http", auth="jwt_vaspmo",
    ...
    # --------------------------------------------------------- notify rules
    @http.route("/vaspmo/api/admin/notify-rules", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    @_guard
    def notify_rules(self, **kw):
        records = request.env["custom.project.notify.rule"].with_context(
            active_test=False
        ).search([])
        return ok([{
            "id": r.id,
    ...
    @http.route("/vaspmo/api/admin/notify-rules/<int:rule_id>", type="http",
                auth="jwt_vaspmo", methods=["POST", "PATCH"], csrf=False, save_session=False)
    @_guard
    def notify_rule_write(self, rule_id, **kw):
        record = request.env["custom.project.notify.rule"].with_context(
            active_test=False
        ).browse(rule_id)
        if not record.exists():
    ...
    # ----------------------------------------------------------------- users
    @http.route("/vaspmo/api/admin/users", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    @_guard
    def users(self, **kw):
        groups = {
        ...
        for user in members:
            ...
            rows.append({
                ...
                "roles": [
                    key for key, xmlid in groups.items() if user.has_group(xmlid)
                ],
