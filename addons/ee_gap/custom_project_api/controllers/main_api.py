...
TASK_WRITABLE = {
    "name", "description", "date_deadline", "custom_priority", "custom_story_points",
    "custom_task_type", "custom_due_sla_date", "custom_hold_reason", "custom_hold_until",
    "depend_on_ids",
    "custom_verification_owner_id", "custom_sprint_id", "custom_vertical_id",
    "custom_vertical_override", "custom_vertical_override_reason", "project_id",
    "stage_id", "user_ids",
}
...
def _vertical(record):
    ...
    if not vertical:
        return None
    return {"id": vertical.id, "code": vertical.code, "name": vertical.name,
...
def _task_dict(task):
    return {
        ...
        "cr_id": task.change_request_id.id
        if "change_request_id" in task._fields and task.change_request_id else None,
...
class VaspmoApi(http.Controller):
    ...
    # ------------------------------------------------------------------ meta
    @http.route("/vaspmo/api/health", type="http", auth="public", methods=["GET"],
                csrf=False, save_session=False)
    def health(self, **kw):
        return ok({"status": "up", "db": request.env.cr.dbname})

    @http.route("/vaspmo/api/meta/stages", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def stages(self, **kw):
    ...
    @http.route("/vaspmo/api/meta/verticals", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def verticals(self, **kw):
        verticals = request.env["custom.project.vertical"].search([])
        return ok([{
            "id": v.id, "code": v.code, "name": v.name,
            "legal_entity": v.legal_entity or "", "brand_group": v.brand_group,
            "color": v.color, "project_count": v.project_count, "task_count": v.task_count,
        } for v in verticals])

    # -------------------------------------------------------------- projects
    @http.route("/vaspmo/api/projects", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    ...
    # ----------------------------------------------------------------- tasks
    @http.route("/vaspmo/api/tasks", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def tasks(self, **kw):
        domain = []
    ...
    @http.route("/vaspmo/api/tasks/<int:task_id>", type="http", auth="jwt_vaspmo",
    ...
    def _audit_for(self, record, limit=40):
        rows = request.env["pdp.audit.log"].sudo().search([
    ...
    @http.route("/vaspmo/api/tasks", type="http", auth="jwt_vaspmo", methods=["POST"],
                csrf=False, save_session=False)
    @_guard
    ...
    @http.route("/vaspmo/api/tasks/<int:task_id>", type="http", auth="jwt_vaspmo",
                methods=["POST", "PATCH"], csrf=False, save_session=False)
    @_guard
    ...
    @http.route("/vaspmo/api/tasks/<int:task_id>/stage", type="http", auth="jwt_vaspmo",
    ...
    @http.route("/vaspmo/api/tasks/<int:task_id>/comment", type="http", auth="jwt_vaspmo",
                methods=["POST"], csrf=False, save_session=False)
    @_guard
    ...
    # ------------------------------------------------------- change requests
    @http.route("/vaspmo/api/change-requests", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    def crs(self, **kw):
        domain = []
        if kw.get("state"):
    ...
    @http.route("/vaspmo/api/change-requests/<int:cr_id>/action", type="http",
                auth="jwt_vaspmo", methods=["POST"], csrf=False, save_session=False)
    @_guard
    ...
    # ---------------------------------------------------------------- weekly
    @http.route("/vaspmo/api/weekly", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def weekly(self, **kw):
        domain = []
        if kw.get("week"):
    ...
    @http.route("/vaspmo/api/weekly/<int:weekly_id>", type="http", auth="jwt_vaspmo",
                methods=["POST", "PATCH"], csrf=False, save_session=False)
    @_guard
    ...
    # ------------------------------------------------------------- dashboard
    @http.route("/vaspmo/api/dashboard/summary", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
    def dashboard_summary(self, **kw):
        ...
        return ok({
            ...
            "tasks_waiting_user": Task.search_count(
            ...
            "cr_waiting_approval": CR.search_count(
                [("approval_state", "=", "waiting_approval")]
            ),
            "cr_active": CR.search_count([("approval_state", "=", "approved")]),
        })

    @http.route("/vaspmo/api/dashboard/ba-summary", type="http", auth="jwt_vaspmo",
                methods=["GET"], csrf=False, save_session=False)
        ...
        for ba in bas:
            ...
            rows.append({
                ...
                "cr_waiting_analysis": CR.search_count([
                    ("ba_id", "=", ba.id), ("approval_state", "=", "draft"),
                ]),
                "tasks_open": Task.search_count([
                    ("user_ids", "in", ba.id),
                    ("stage_id.custom_is_closed_stage", "=", False),
                ...
                "carry_over": Task.search_count([
                    ("user_ids", "in", ba.id), ("custom_carried_over", "=", True),
                ]),
                "avg_cycle_time_team": round(
                    sum(cycle_values) / len(cycle_values), 2
                ) if cycle_values else 0.0,
                "weekly_submitted": Weekly.search_count([
                    ("author_id", "=", ba.id), ("sprint_id", "=", sprint.id),
                    ("state", "!=", "draft"),
                ]),
    ...
    @http.route("/vaspmo/api/search", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def search(self, **kw):
    ...
    @http.route("/vaspmo/api/logs", type="http", auth="jwt_vaspmo", methods=["GET"],
                csrf=False, save_session=False)
    def logs(self, **kw):
