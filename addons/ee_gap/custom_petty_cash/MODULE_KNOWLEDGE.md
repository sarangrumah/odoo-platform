...
- **A cash account is a legal advance account.** Sheet #44 asked for `1102000001 Cash on hand` next to `1102000099 Petty Cash Clearing`, and the picker's `[('reconcile','=',True)]` domain hid it — a cash account is `asset_cash` with `reconcile` off. The domain now also admits `asset_cash`. Nothing else had to change: `action_settle` already skipped auto-reconcile when the account is not reconcilable, which is the correct behaviour for a float account. Marking the cash account `reconcile` instead would have made **every** advance settlement reconcile cash, which is not what a float is.
- **`mapped("account_id").reconcile` was a singleton error waiting to happen.** Two advance lines on two accounts and the settle raises. It is now `all(adv_accounts.mapped("reconcile"))`, and the skip is logged rather than silent.
- **The test fixtures must not assume an empty database.** `common.py` hard-coded petty cash type codes `CA` and `PC` and `is_default=True`; on any tenant that actually runs Petty Cash both collide, and five test classes died in `setUpClass`. Codes are now `T-` prefixed and the incumbent default is stood down inside the test transaction — 3 runnable tests became 42. `TestPettyCashFlow.test_disburse_books_advance_and_bank` still fails on a configured database (it also fails on `origin/main`); it reads the advance line off the company fallback and something else on such a database claims it first.
