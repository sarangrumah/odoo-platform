...
class BankImportCsvWizard(models.TransientModel):
    ...
    template_id = fields.Many2one(
        "custom.bank.import.template", required=True)
    ...
    def action_import(self):
        ...
        if not lines:
            log = Log.create({
                ...
                "error_message": "; ".join(f"row {n}: {e}" for n, e in errors[:50])
                or "No parseable lines.",
            })
            raise UserError(_(
        ...
        log = Log.create({
            ...
            "error_count": len(errors),
            "state": state,
            "raw_payload": "; ".join(f"row {n}: {e}" for n, e in errors[:200])
