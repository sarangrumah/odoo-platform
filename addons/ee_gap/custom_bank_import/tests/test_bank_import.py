...
class _MockH2HAdapter(BaseAdapter):
    ...
    def inquiry_balance(self, account_number):
        return AdapterResponse(ok=True, status_code=200,
                               data={"balance": 1000.0})
...
class TestBankImport(TransactionCase):
    ...
    def test_csv_happy_path(self):
        csv = (
            b"date,ref,amount\n"
            b"2026-05-01,REF001,1500.00\n"
            b"2026-05-02,REF002,-250.00\n"
    ...
    def test_h2h_mock_adapter_creates_lines(self):
        _MockH2HAdapter.fixture_lines = [
            {"date": "2026-05-01", "description": "Salary",
             "ref": "SAL001", "amount": 5000.0},
            {"date": "2026-05-02", "description": "Vendor pay",
             "ref": "VND001", "amount": -1200.0},
        ]
        ...
        conn._do_sync()
        log = self.Log.search([("journal_id", "=", self.journal.id),
                               ("filename", "like", "h2h-Other-%")],
