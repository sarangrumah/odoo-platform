...
class TestBankImport(TransactionCase):
    ...
    def test_bri_mutasi_xlsx_autodetect(self):
        # The other BRI export: real date cells, real number columns, and a
        # header that spells out "Uraian Transaksi" / "Teller ID" — which the
        # BRISIM detector (exact labels) does not match.
        import io as _io
        ...
        wb = openpyxl.Workbook()
        sh = wb.active
        sh.append(["Tanggal", "Uraian Transaksi", "Teller ID", "Debet", "Kredit", "Saldo"])
        sh.append(
            [
                datetime(2026, 9, 1),
                "OffUs 1 260831 001999660762 LEVIS TUNJUN AMT:2.401.800,00MDR:18.014,00",
                "BRIMTXDT",
                0,
                2383786,
                226976052,
        ...
        # A file re-saved through a spreadsheet often carries padded text
        # amounts instead of numbers; same reading either way.
        sh.append(
            [
                datetime(2026, 9, 1),
                "DEBET BY CEK CGZ 006326",
                "BRIMTXDT",
                " 275,000 ",
                " 0 ",
                " 226,701,052 ",
        ...
        self.assertEqual(len(res["lines"]), 2)
        credit, debit = res["lines"]
        self.assertEqual(credit["date"].isoformat(), "2026-09-01")
        self.assertEqual(credit["amount"], Decimal("2383786"))
        self.assertEqual(credit["balance"], Decimal("226976052"))
        self.assertIn("LEVIS TUNJUN", credit["ref"])
        self.assertEqual(debit["amount"], Decimal("-275000"))

    def test_brisim_still_wins_over_bri_mutasi(self):
        # The BRISIM header is a prefix of the mutasi one; the shifted-column
        # export must keep its own handler.
        rows = [["Tanggal", "Uraian", "Teller", "Debet", "Kredit", "Saldo"]]
        self.assertTrue(self.template._is_brisim(rows))
        self.assertTrue(self.template._is_bri_mutasi(rows))
