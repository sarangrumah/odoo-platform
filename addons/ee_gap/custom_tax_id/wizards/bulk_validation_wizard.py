...
class BulkValidationWizard(models.TransientModel):
    ...
    def action_run(self):
        self.ensure_one()
        move_types = (
            ("out_invoice", "out_refund") if self.move_type == "out_invoice"
            else ("in_invoice", "in_refund") if self.move_type == "in_invoice"
        ...
        if sertel_warning:
            html_parts.append(
                f'<div class="alert alert-warning">{sertel_warning}</div>'
            )
        if not errors:
            html_parts.append(
                f'<div class="alert alert-success">'
                f'<b>✓ All {len(moves)} moves pass validation.</b></div>'
            )
        else:
            html_parts.append(
                f'<div class="alert alert-danger">'
                f'<b>{len(errors)} of {len(moves)} moves have issues</b></div>'
            )
            html_parts.append('<table class="table table-sm">')
            html_parts.append(
                '<thead><tr><th>Move</th><th>Partner</th><th>Date</th>'
    ...
    def _check_move(self, move) -> list[str]:
        ...
        if move.move_type in ("out_invoice", "out_refund"):
            ...
            if npwp_status == "invalid":
                issues.append(
                    f"Partner NPWP '{partner.x_custom_npwp}' invalid (must be 15 or 16 digits)."
                )
        else:
