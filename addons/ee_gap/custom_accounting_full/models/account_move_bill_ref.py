# -*- coding: utf-8 -*-
"""Hard-block duplicate vendor bill references at posting time.

Odoo CE only *warns* about a re-used Bill Reference (via the native
``duplicated_ref_ids`` banner) — the bill still posts. Group policy here
requires vendor bill references to be unique per vendor, so we escalate
that warning to a blocking ``UserError`` raised inside ``_post``.
