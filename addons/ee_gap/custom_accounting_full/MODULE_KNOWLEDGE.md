...
- **The Lock Dates wizard writes with `sudo()`.** `res.company` is writable only by Settings admins, but closing a period is an accountant's job — the gate is the explicit `account.group_account_manager` check in `action_apply` plus the ACL on the wizard model, not the ORM. Removing either one hands lock dates to any user who can reach the model.
- **The wizard's date fields are plain fields, not `readonly=False` computes.** A non-stored compute has nowhere to keep what the user types, so `create()` drops it silently and the wizard appears to do nothing. Defaults seed the current values; `_onchange_company_id` reloads them when the company changes.
- **Any action dict handed to `RedirectWarning` must carry `views`.** `doAction` receiving an *object* returns it from `_loadAction` untouched — `views` is never derived from `view_mode` the way `/web/action/load` would for a stored `ir.actions.act_window`. `_preprocessAction` then runs `action.views.map(...)` on `undefined` and the button dies with an uncaught `TypeError`, leaving the user staring at a warning they cannot act on. Core's own `_get_unreconciled_statement_lines_redirect_action` forgets this, which is why `res_company_inherit.py` overrides it to fill `views` in from `view_mode`; the hard-lock-date redirect thirty lines above it in the same core method spells them out correctly. Reported from `eal-hub.erajaya.com`, 12-Aug-2026.
- **A lock date is not the last word — `account.lock_exception` is.** Core enforces `_get_user_lock_date()`, which lets an exception lower the date for one user, or for everyone when it carries no `user_id`; an exception with no `end_datetime` never lapses. Neither the company form nor this wizard showed them, so a period could read as closed while named people were still free to post into it. Measured on prd_levis_begbal 12-Aug-2026: the company said 31-Jul while **six users sat at 31-May**, deliberately and with authority, but invisibly. The wizard now lists every active exception with its user, date, expiry and reason. Two traps behind it: writing a new lock date does **not** clear exceptions — core re-creates each active one against the new company date, which is why the company chatter shows a burst of "31-Jul → 31-May" entries alongside the real change — and `reason` is free text rendered into a `sanitize=False` field, so it is escaped.
- **The Lock Dates wizard pre-flights unreconciled statement lines.** Core refuses a Global Lock Date while any bank statement line in the period is `is_reconciled = False`, and its RedirectWarning is a dead end — the user has already filled the form in. `unreconciled_stmt_count` counts them before Apply and offers its own link, so the warning is information rather than a bounce. Note the suspense account is `reconcile = False` by Odoo's own design, so these lines clear by having their suspense leg *replaced*, never matched.
