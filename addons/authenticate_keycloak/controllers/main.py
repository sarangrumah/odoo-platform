...
class AuthenticateKeycloakLogin(Home):
    ...
    def web_login(self, *args, **kw):
        ...
        if response.is_qweb:
            ...
            response.qcontext["keycloak_css_class"] = (
                request.env["ir.config_parameter"].sudo().get_param("authenticate_keycloak.css_class")
            )
            response.qcontext["keycloak_button_label"] = (
                request.env["ir.config_parameter"].sudo().get_param("authenticate_keycloak.button_label")
            )
            if error:
...
class AuthenticateKeycloakController(http.Controller):
    ...
    def signin(self, **kw):
        ...
        try:
            ...
            if user.employee_id:
                ...
                _logger.info(
                    f"User {user.login} is linked to employee {employee_to_update.name}. Checking for updates."
                )
                update_vals = {}
                ...
                _logger.info(
                    f"User {user.login} is not linked to an employee. Searching for existing employee with email {user.login}."
                )
                ...
                if employee:
                    ...
                    _logger.info(
                        f"Found existing employee '{employee.name}' (ID: {employee.id}). Linking to user and updating."
                    )
                    ...
                    _logger.warning(
                        f"No existing employee found with email {user.login}. User will not be linked to an employee record."
                    )
            # --- END SYNC LOGIC ---
    ...
    def _sync_employee_data_if_needed(self, employee):
        ...
        if employee.nik:
            if not all([employee.department_id, employee.job_id, employee.parent_id]):
                _logger.info(f"Employee {employee.id} is missing some data. Triggering API sync.")
                self._sync_employee_data_from_api(employee)
        else:
            _logger.info(f"Employee {employee.id} has no NIK. Skipping API sync.")
    ...
    def _sync_employee_data_from_api(self, employee):
        ...
        try:
            ...
            _logger.info(f"Employee Sync: Getting data for employee {employee.id}")
            ...
            if not response_data.get("status"):
                _logger.error(
                    f"Employee Sync API Error for employee {employee.id}: {response_data.get('message', 'Unknown error')}"
                )
                return
            ...
            if superior_data and superior_data.get("email") and not employee.parent_id:
                ...
                superior_employee = (
                    request.env["hr.employee"].sudo().search([("work_email", "=", superior_email)], limit=1)
                )
                if superior_employee and superior_employee.id != employee.id:
                    update_vals["parent_id"] = superior_employee.id
                else:
                    _logger.warning(f"Employee Sync: Superior not found or is self for employee {employee.id}.")

            if update_vals:
                employee.sudo().write(update_vals)
                _logger.info(f"Employee Sync: Successfully synced data for employee {employee.id}: {update_vals}")

        except requests.exceptions.RequestException as e:
            _logger.error(f"Employee Sync: API request failed for employee {employee.id}: {_scrub(e, employee.nik)}")
        except Exception as e:
            _logger.error(
                f"Employee Sync: An unexpected error occurred for employee {employee.id}: {_scrub(e, employee.nik)}"
            )
