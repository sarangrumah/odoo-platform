...
class ResUsers(models.Model):
    ...
    is_keycloak_user = fields.Boolean(
        string="Keycloak User", default=False, help="User authenticated through Keycloak SSO"
    )

    keycloak_sub = fields.Char(string="Keycloak Subject ID", help="Unique identifier from Keycloak")
        ...
        if password is None:
            with cls.pool.cursor() as cr:
                ...
                user = env["res.users"].search(
                    [("login", "=", login), ("is_keycloak_user", "=", True), ("active", "=", True)], limit=1
                )
