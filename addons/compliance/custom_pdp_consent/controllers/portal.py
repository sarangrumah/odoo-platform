...
class PdpConsentPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "consent_count" in counters:
            ...
            values["consent_count"] = request.env["pdp.consent"].sudo().search_count([
    ...
    def portal_my_consents(self, **kw):
        partner = request.env.user.partner_id
        consents = request.env["pdp.consent"].sudo().search([
            ("partner_id", "=", partner.id),
        ], order="given_at desc")
    ...
    @http.route(["/my/consents/<int:consent_id>/withdraw"], type="http",
