...
class TestPosClearing(AccountTestInvoicingCommon):
    ...
    def test_the_receipts_pick_the_tender_the_residual_ordering_would_not(self):
        ...
        run = self._run()
        with patch.object(type(self.env["levis.pos.clearing.alloc"]), "_x24_rows", lambda self, *args, **kwargs: rows):
            run.action_compute()
    ...
    def test_the_trading_day_follows_the_mutation_not_a_matching_neighbour(self):
        """Production, 1 September 2026: a QRIS settlement moved to the wrong day.

        The bank moved 1.550.900 and its narrative said 31/08. Nothing that store
        sold on the 31st added up to it, but the 30th happened to hold a CASH sale
        of exactly that amount, so the trading day walked back a day and the line
        then reported a tender conflict against a day it never traded on. The day
        now follows the mutation; the receipts only order the allocation.
        """
        receivable = self._posrec(self.tender_a, self.store_one, date(2026, 7, 5), 1_550_900.0)
        self._statement(
            date(2026, 7, 6), 1_550_900.0, self._settlement_ref(MID_ONE, 1_550_900.0, 0.0, date(2026, 7, 5))
        )
        self.tender_c.name = "POS Receivable - CASH"
        rows = {(self.store_one.id, date(2026, 7, 4)): [("CASH", "80433-1-1550", 1_550_900.0)]}
        run = self._run()
        with patch.object(type(self.env["levis.pos.clearing.alloc"]), "_x24_rows", lambda self, *args, **kwargs: rows):
            run.action_compute()
        line = run.line_ids.filtered(lambda item: item.block == "a")
        self.assertEqual(line.trans_date, date(2026, 7, 5), "the mutation date minus the settlement lag")
        self.assertFalse(line.trans_date_is_derived, "the narrative names that same day")
        self.assertEqual(line.alloc_ids.source_aml_id, receivable)

    def test_a_narrative_day_that_disagrees_with_the_mutation_is_flagged_not_followed(self):
        """The bank's own date is a cross-check, never the trading day itself."""
        self._posrec(self.tender_a, self.store_one, date(2026, 7, 5), 400_000.0)
        self._statement(date(2026, 7, 6), 400_000.0, self._settlement_ref(MID_ONE, 400_000.0, 0.0, date(2026, 7, 3)))
        run = self._run()
        run.action_compute()
        line = run.line_ids.filtered(lambda item: item.block == "a")
        self.assertEqual(line.trans_date, date(2026, 7, 5))
        self.assertTrue(line.trans_date_is_derived, "the narrative says 03/07, which is not the anchored day")
    ...
    def test_a_proven_settlement_takes_its_receivable_before_an_unproven_one(self):
        ...
        run = self._run()
        with (
            patch.object(type(self.env["levis.pos.clearing.alloc"]), "_x24_rows", only_for_the_later_line),
            patch.object(
                type(run),
                "_allocation_order",
