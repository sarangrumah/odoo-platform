...
def _unique_journal_code(env, company, base):
    ...
    ``account.journal.code`` is five characters, so the counter has to eat into
    the stem as it grows. Keeping a fixed four-character stem and appending ``n``
    does not: from ``n=10`` every candidate truncates back to ``stem`` + the
    counter's FIRST digit, so ``n=10..19`` all re-emit the same five characters
    ``n=1`` already tried. Once those ten variants are taken the ``while`` loop
    never terminates — it spins on nine values forever.

    That is not theoretical. Provisioning nine consecutive store codes in one run
    (80741…80748) fills ``8074x`` with cash journals, and the tenth warehouse's
    purchase journal then hangs the whole transaction with no error and no log.

    So shrink the stem by the width of the counter, which makes every ``n`` a
    distinct candidate, and raise rather than loop if the space is ever
    exhausted. Every input that terminated before still returns the same code:
    ``n`` in 1..9 yields ``base[:4] + n`` exactly as it always did.
    """
    ...
    if not _taken(base[:5]):
        return base[:5]
    for n in range(1, 100000):
        suffix = str(n)
        candidate = (base[: max(0, 5 - len(suffix))] + suffix)[:5]
        if not _taken(candidate):
            return candidate
    raise UserError(
        _(
            "No free 5-character journal code is left near %(base)s in %(company)s.",
            base=base,
