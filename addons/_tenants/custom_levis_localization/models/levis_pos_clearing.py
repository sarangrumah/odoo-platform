...
class LevisPosClearing(models.Model):
    ...
    store_day_ids = fields.One2many("levis.pos.clearing.store.day", "run_id", copy=False)
    ...
    def _compute_one(self):
        ...
        # The same projection cut the other way — per store, against the X70D
        # tenders of the trading day. It reads the lines and their allocations,
        # so it follows the days rather than racing them.
        self.env["levis.pos.clearing.store.day"]._rebuild_for_run(self)
        self.state = "computed"
        return True
    ...
    def action_view_store_days(self):
        """The run's settlements cut by store and date, against the X70D day.

        Rebuilt on the way in rather than only at compute time. A run computed
        before this screen existed has no store rows, and the only other way to
        get them would be to recompute the month — which throws away every
        receipt tick the accountant has made. The projection books nothing, so
        rebuilding it costs nothing but a second.
        """
        StoreDay = self.env["levis.pos.clearing.store.day"]
        for run in self.filtered(lambda run: run.state != "draft"):
            StoreDay._rebuild_for_run(run)
        return {
            "type": "ir.actions.act_window",
            "name": _("Store Settlement Reconcile"),
            "res_model": "levis.pos.clearing.store.day",
            "view_mode": "list,form",
            "domain": [("run_id", "in", self.ids)],
            "context": {"create": False, "search_default_group_store": 1},
        }
    ...
    def _resolve_target(self, statement_line, parsed, rules):
        ...
        # The trading day is anchored on the bank mutation, never on the date the
        # narrative quotes. One rule for every row: money that landed on 1 Sep
        # settles the trading day one lag earlier, which is the day whose POS
        # receivable is open. The narrative's "TANGGAL" is the acquirer's batch
        # date; it is missing on most rows, so honouring it where present only
        # made two kinds of row behave differently.
        primary = statement_line.date - timedelta(days=self.config_id.settlement_lag_days or 0)
        # The narrative still earns its keep as a cross-check: a day the bank
        # spells out and the anchor agree on is corroborated, not inferred.
    ...
    def _attach_evidence(self, prepared, pos_accounts):
        ...
        The narrative cannot say which tender account a settlement belongs to —
        one card MID covers Visa, Mastercard, JCB and Amex — so the account used
        to be whichever had the largest residual. Where a day's transactions
        identify the settlement by arithmetic, they name the tender outright, and
        that beats a residual ordering: measured on August 2026, 377 settlements
        had their tender named by the receipts and a *different* account credited
        by the allocation.

        The search still walks the candidate days, because a receivable can be
        booked a day either side of the settlement, but what it finds only orders
        the allocation. It never renames the trading day, which stays anchored on
        the bank mutation — see ``_resolve_target``.

        Nothing here is forced: a settlement nothing proves keeps the old
        behaviour exactly.
        """
        self.ensure_one()
        Alloc = self.env["levis.pos.clearing.alloc"]
        ...
        for prep in settling:
            ...
            for day in self._candidate_dates(prep["trans_date"]):
                ...
                # The proven day steers the allocation, but it does NOT become the
                # line's trading day: an amount matching on a neighbouring day is
                # arithmetic, not proof of when the sale happened, and a QRIS
                # settlement has been seen to land on a same-amount CASH receipt a
                # day earlier. The trading day stays anchored on the mutation.
                break
        return prepared
...
class LevisPosClearingLine(models.Model):
    ...
    trans_date_is_derived = fields.Boolean(
        ...
        help="The trading day is always the mutation date minus the settlement "
        "lag. This is ticked when the bank narrative did not name that same day — "
        "either it named no day at all, or it named a different one. The account "
        "and the store are still exact.",
    )
    ...
    store_day_id = fields.Many2one(
        ...
        string="Store Settlement Day",
        index="btree_not_null",
        ondelete="set null",
        copy=False,
        help="The store-and-date this line was rolled up into for the store "
        "reconciliation. A projection, like Clearing Day: nothing about the "
    ...
    def action_open_store_day(self):
        """This line's store-and-date row: the money in, against the store's day."""
        self.ensure_one()
        if not self.store_day_id:
            raise UserError(
                _(
                    "This settlement has no store yet, so there is nothing to "
                    "reconcile against a trading day. Map its merchant id first."
                )
            )
        return {
            "type": "ir.actions.act_window",
            "res_model": "levis.pos.clearing.store.day",
            "res_id": self.store_day_id.id,
            "view_mode": "form",
        }

    def action_open_store_days(self):
        """The store-and-date rows behind a selection of settlements.

        Builds them first where a run has none: same reason as on the run
        button — recomputing to get a projection would cost the ticks.
        """
        StoreDay = self.env["levis.pos.clearing.store.day"]
        pending = self.filtered(
            lambda line: line.analytic_account_id and line.settlement_date and not line.store_day_id
        )
        for run in pending.mapped("run_id").filtered(lambda run: run.state != "draft"):
            StoreDay._rebuild_for_run(run)
        rows = self.mapped("store_day_id")
        if not rows:
            raise UserError(_("None of the selected settlements names a store, so there is nothing to reconcile."))
        return {
            "type": "ir.actions.act_window",
            "name": _("Store Settlement Reconcile"),
            "res_model": "levis.pos.clearing.store.day",
