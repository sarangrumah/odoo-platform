...
def ellipse(size, pos, colour, extra=None):
    it = [
        ...
        {"ty": "tr", "p": val([0, 0]), "a": val([0, 0]), "s": val([100, 100]), "r": val(0), "o": val(100)},
    ]
    if extra:
        it[2].update(extra)
        ...
        "it": [
            {
                "ty": "rc",
                "p": val(pos),
                "s": size if isinstance(size, dict) else val(size),
                "r": val(radius),
                "nm": "r",
            },
            {"ty": "fl", "c": val(colour), "o": val(100), "r": 1, "nm": "f"},
...
def layer(ind, name, shapes, *, pos=[100, 100, 0], scale=None, rot=None, opacity=None, ip=0, op=OP, anchor=[0, 0, 0]):
    return {
        "ddd": 0,
        "ind": ind,
        "ty": 4,
...
add("mouth-rest", [rect([34, 6], [0, 0], NEAR_BLACK, radius=3)], pos=[100, 126, 0], ip=0, op=180)

# --- thinking: three dots orbiting above the head ----------------------------
for i, dx in enumerate((-18, 0, 18)):
    add(
        f"think-{i}",
        [ellipse([12, 12], [0, 0], INDIGO_DARK)],
        pos=[100 + dx, 34, 0],
        ip=120,
        op=180,
        scale=anim(
...
add(
    "antenna-stem",
    [rect([5, 22], [0, 11], INDIGO_DARK, radius=2)],
    pos=[100, 40, 0],
    anchor=[0, 22, 0],
