...
const GREETING =
  "Halo. Saya membaca data POS Levi's di dashboard ini. Tanya saja angkanya.";
...
/**
 * The app is mounted under basePath "/cockpit", which next/link prefixes for us
...
export function AgentWidget() {
  ...
  async function ask(question: string) {
    ...
    try {
      const res = await fetch(`${apiBase}/api/agent`, {
        ...
        // The filter bar IS the reader's context: send it so a question with no
        // period in it answers about the range already on screen.
        body: JSON.stringify({ question: text, filters: Object.fromEntries(params) }),
      });
  ...
  return (
    <>
      ...
      {open && (
        <section className="agent-panel" role="dialog" aria-label="Asisten data">
          <header className="agent-head">
            <Mascot state={state} size={32} />
            <div>
              <strong>Asisten Cockpit</strong>
              <span>data POS prd_levis_begbal</span>
            </div>
          ...
          >
            <input
              ...
              placeholder="Tanya angka penjualan…"
              maxLength={500}
              disabled={busy}
            />
