...
const ODOO_URL = (process.env.COCKPIT_ODOO_URL ?? "http://odoo-front:8069").replace(/\/+$/, "");
const DB = process.env.COCKPIT_DB_NAME ?? "prd_levis_begbal";
const TTL_SECONDS = Number(process.env.COCKPIT_SESSION_TTL ?? 60 * 60 * 12);
...
function secret(): string {
  const value = process.env.COCKPIT_SESSION_SECRET;
  // Fail closed. A default secret would mean anyone who read this file could
  // forge a session cookie.
  if (!value || value.length < 32) {
    throw new Error("COCKPIT_SESSION_SECRET is missing or shorter than 32 characters");
  }
  return value;
}
