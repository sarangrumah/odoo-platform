...
Usage:  python docs/projects/levis/build_fsd_docx.py
Output: docs/projects/levis/FSD - Levis Retail Odoo (SES x EBR) v1.0.docx
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "FSD - Levis Retail Odoo (SES x EBR) v1.0.docx")
...
def p(text, bold=False, italic=False, align=WD_ALIGN_PARAGRAPH.JUSTIFY, size=None,
...
for _ in range(5):
    doc.add_paragraph()
p("FUNCTIONAL SPECIFICATION DOCUMENT", bold=True, align=WD_ALIGN_PARAGRAPH.CENTER,
  size=22, color=ACCENT, space_after=2)
p("(FSD)", bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, size=14, color=ACCENT)
doc.add_paragraph()
p("Implementasi Odoo untuk Bisnis Retail Levi's Indonesia", bold=True,
  align=WD_ALIGN_PARAGRAPH.CENTER, size=16)
p("PT Sinar Eka Selaras (SES — SAP)  ×  PT EBR (Retail Levi's — Odoo)  ×  POS Principal",
...
p("Dokumen ini diajukan untuk mendapatkan persetujuan atas rancangan fungsional "
  ...
  "change request.")
table(["Peran", "Nama", "Jabatan / Fungsi", "Tanda Tangan", "Tanggal"], [
...
table(["Versi", "Tanggal", "Penyusun", "Deskripsi Perubahan"], [
...
p("Erajaya Group melalui PT Sinar Eka Selaras (SES) bertindak sebagai distributor resmi "
...
p("Dokumen ini mendefinisikan kebutuhan fungsional (functional requirements) implementasi "
...
p("Rantai proses melibatkan empat aktor dengan tiga sistem yang berbeda. Levi's Principal "
...
p("PT SES menerbitkan Purchase Order kepada Levi's Principal melalui SAP. Principal "
...
table(["Langkah", "Aktivitas", "Sistem"], [
...
p("PT EBR menerbitkan Purchase Order kepada PT SES melalui Odoo. SES menerima PO tersebut, "
...
p("Seluruh transaksi operasional toko terjadi pada POS bawaan principal, meliputi goods "
...
table(["ID", "Kebutuhan Fungsional", "Deskripsi", "Prioritas"], [
    ...
    ["FR-04", "Mirror penjualan POS (X24DN)",
     ...
     "keputusan Phase 5; executor menahan posting hingga diaktifkan.", "P2"],
    ...
    ["FR-11", "SFTP feed otomatis",
     ...
     "notifikasi bila parsing gagal.", "P2"],
    ...
    ["FR-13", "Pelaporan keuangan",
     ...
     "transaksi (Invoice, Quotation, PO) menggunakan template PDF ber-branding per company.", "M"],
    ["FR-14", "Pajak Indonesia",
     "PPN keluaran/masukan dikonfigurasi sesuai tarif berlaku; integrasi Coretax/e-Faktur "
     "dan bukti potong melalui custom_coretax dan custom_coretax_bupot.", "M"],
], widths=[1.6, 4.0, 8.6, 2.0])
doc.add_page_break()

# ===================== 4. MODUL & KUSTOMISASI ===============================
doc.add_heading("4. Modul Odoo yang Diinstal dan Kustomisasi", level=1)
p("Tenant EBR diprovision menggunakan Industry Pack \"Retail / POS\" pada platform Odoo "
...
table(["Modul", "Fungsi pada Implementasi Ini"], [
...
table(["Komponen", "Fungsi"], [
    ...
    ["queue_job (OCA)", "Eksekusi import file besar secara asinkron (X101 ± 159 ribu SKU) tanpa terputus timeout proxy."],
...
p("Modul custom_retail_import dibangun untuk onboarding Levi's dan dirancang reusable untuk "
...
bullets([
    ...
    ("Eksekusi asinkron. ",
     "File besar diproses di background melalui queue_job."),
    ("Phase-5 gating. ",
...
p("Report standar dari sistem principal berikut menjadi sumber data mirroring ke Odoo. "
  "Seluruh file berformat Excel/CSV pada Phase 1 dan dikirim melalui FTP/SFTP Erajaya pada "
  "Phase 2 tanpa perubahan struktur.")
table(["Report / File", "Isi Data", "Tujuan di Odoo", "Frekuensi"], [
    ...
    ["X20_Current_Onhand_Inventory", "Posisi stok on-hand per toko.",
     "Saldo awal (one-shot) dan validasi stok/inventory adjustment.", "Harian / periodik"],
    ["X24DN_Retail_Sales_Detail", "Detail transaksi penjualan POS per toko.",
     "Pencatatan penjualan (jurnal penjualan + HPP).", "Harian (EOD)"],
    ["X70D_Tender_Detail / X70T_Tender_Settlement",
     "Rincian metode pembayaran (tunai, EDC, e-wallet) dan settlement-nya.",
     "Payment register dan rekonsiliasi bank.", "Harian (EOD)"],
    ["X32P_Stock_Movement_With_Price",
     "Mutasi persediaan bernilai: GR toko, transfer, retur, adjustment.",
     "Stock move dan receipt (termasuk GR atas PO EBR → SES).", "Harian / periodik"],
    ["X31_Discount_Journal", "Data diskon/promo.",
     "Dasar klaim promosi kepada principal/SES.", "Bulanan / per program"],
    ["Store Master", "Daftar 24 lokasi (1 DC + 23 toko).",
     "Pembentukan warehouse per toko dan crosswalk kode toko.", "Saat ada perubahan"],
    ["CoA EBR", "Bagan akun (code, name, account_type).",
     "Chart of Accounts PT EBR.", "Satu kali (go-live)"],
], widths=[4.4, 4.6, 4.7, 2.5])

# ===================== 6. KONFIGURASI DATA ==================================
doc.add_heading("6. Konfigurasi Data dan Urutan Setup", level=1)
p("Penyiapan data tenant EBR mengikuti urutan berikut: konfigurasi dasar terlebih dahulu "
...
table(["No", "Data", "File / Sumber", "Isi yang Dibutuhkan", "Cara Load"], [
    ["1", "Company Detail (EBR)",
    ...
    ["2", "Chart of Accounts",
     ...
     "Kolom code, name, account_type (enum Odoo); mencakup akun persediaan, HPP, penjualan, "
     "kas/bank, AP/AR.",
     "Profil import levis_coa atau Accounting ▸ CoA ▸ Import."],
    ["3", "Bank Account & Journals",
    ...
    ["5", "Vendor: PT SES",
     ...
     "SES sebagai vendor pada Odoo EBR: NPWP, alamat, term of payment, rekening tujuan "
     "pembayaran.",
     "Manual atau import partner (satu kali)."],
    ["6", "Master Produk",
     "X101_Material_Master.xlsx (full: 14.885 artikel / 159.658 SKU; atribut Size 89 dan "
     "Inseam 24).",
     "Artikel, varian (size/inseam), barcode, kategori (170), harga.",
     "Profil import levis_x101 (asinkron, ± 30 menit)."],
    ["7", "Store Master → Warehouse",
    ...
    ["8", "Saldo Awal Stok",
     ...
     "On-hand per toko per SKU pada tanggal cut-over (one-shot; dikunci agar tidak terimpor "
     "dua kali).",
...
p("Pada fase ini tim toko/operasional mengekspor report dari sistem principal dalam format "
...
p("Pada fase ini file report di-drop secara otomatis ke server FTP/SFTP Erajaya sesuai "
...
bullets([
    ...
    "Akses server FTP/SFTP Erajaya (host, kredensial, struktur folder) disediakan sebelum "
    "Phase 2 dimulai.",
])
doc.add_heading("8.2 Isu Terbuka (Harus Diselesaikan Sebelum Go-Live Penuh)", level=2)
table(["No", "Isu", "Dampak", "Tindak Lanjut"], [
    ["1", "File X20/X24/X70D yang diterima masih sampel satu toko (store 14694 — Tunjungan "
          "Plaza 3).",
     "Saldo awal dan penjualan hanya dapat dimuat untuk satu toko.",
     "Minta ekstrak lengkap seluruh toko kepada principal/SES, atau aktifkan langsung feed "
     "SFTP."],
    ["2", "Store Master memuat nama toko tanpa kode toko untuk 23 dari 24 lokasi.",
     "Crosswalk kode toko → warehouse tidak lengkap; file X tidak dapat di-join untuk 23 toko.",
     "Minta store master lengkap dengan kode SAP; proses load store bersifat idempoten dan "
     "dapat diulang."],
    ["3", "Keputusan Phase 5 belum ditetapkan: representasi dokumen POS, kedalaman histori "
     ...
     "Workshop keputusan bersama Finance EBR dan tim pajak; setelah ditetapkan, gating "
     "dibuka."],
], widths=[1.0, 5.4, 4.6, 5.2]),

p("Dengan disetujuinya dokumen ini, tim implementasi akan melanjutkan ke tahap konfigurasi, "
