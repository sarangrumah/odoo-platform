...
Usage:  python docs/projects/levis/build_tsd_charter_workflow_docx.py
...
SUBTITLE = ("PT Sinar Eka Selaras (SES — SAP)  ×  PT EBR (Retail Levi's — Odoo)"
            "  ×  POS Principal")
...
def build_tsd():
    b = Builder()
    b.cover("TECHNICAL SPECIFICATION DOCUMENT",
            "TSD — Implementasi Odoo untuk Bisnis Retail Levi's Indonesia",
            SUBTITLE, "TSD/ERAJAYA/LEVIS-ODOO/2026/002")
    b.approval(APPROVAL_INTRO, APPROVAL_ROWS)

    # ---- 1. Pendahuluan ----
    b.h("1. Pendahuluan", 1)
    b.h("1.1 Tujuan Dokumen", 2)
    b.p("Dokumen ini menjabarkan spesifikasi teknis implementasi Odoo untuk PT EBR "
    ...
    b.h("2.1 Platform", 2)
    b.p("Odoo 19 Community Edition berjalan di atas platform multi-tenant Erajaya dengan "
    ...
    b.table(["Komponen", "Versi/Sumber", "Kegunaan"], [
        ["custom_retail_import", "19.0.0.1.0 (addons/ee_gap)",
        ...
        ["Modul dependensi", "custom_core, product, stock, account",
         "Dependensi manifest custom_retail_import."],
    ], widths=[4.2, 4.2, 7.8])
    b.p("Library openpyxl dan paramiko ditambahkan pada odoo/requirements.txt dan tersedia "
    ...
    b.p("Phase 1: pengguna mengunduh report dari sistem POS principal, lalu mengunggahnya "
    ...
    b.p("Modul terdiri atas lima komponen: profil import (konfigurasi parsing per jenis "
    ...
    b.p("Mendeklarasikan cara mem-parsing satu jenis file sumber. Mendukung 10 jenis file: "
    ...
    b.table(["Field", "Tipe", "Keterangan"], [
        ...
        ["namespace", "Char (default retail_import)",
        ...
        ["data_start_row", "Integer (1-based)",
         "Baris pertama DATA; baris judul/header di atasnya dilewati. X101=3, Store Master=3, "
         "X24/X70D=2."],
        ["encoding / delimiter", "Selection / Char",
         "CSV: utf-8, utf-8-sig (default, BOM SSRS), latin-1; delimiter default ','."],
        ["column_map_json", "Text (JSON)",
         "Pemetaan nama field logis → indeks kolom 1-based (A=1), mis. "
         "{\"product_code\": 2, \"sku\": 10}."],
        ...
        ["fix_encoding", "Boolean (default True)",
    ...
    b.table(["Field", "Tipe", "Keterangan"], [
        ...
        ["attachment_id", "M2O ir.attachment",
         "File sumber diarsipkan untuk audit dan pemrosesan ulang."],
    ...
    b.table(["Field", "Tipe", "Keterangan"], [
        ...
        ["last_run / last_status / last_message / files_imported", "Status",
         "Telemetri polling terakhir (ok/error/idle) untuk monitoring."],
    ], widths=[4.8, 3.4, 8.0])

    b.h("3.1.4 retail.import.wizard dan retail.import.executor", 3)
    b.p("retail.import.wizard adalah model transient untuk unggah file (Binary), dengan "
    ...
    b.p("Method read_records(file_b64, limit) pada profil mengubah byte file menjadi daftar "
    ...
    b.bullets([
        ...
        ("Pembersihan string. ",
         "Trim whitespace dan perbaikan encoding U+FFFD → '®' bila fix_encoding aktif."),
    ...
    b.p("Perlindungan duplikasi diterapkan pada dua lapis: lapis file (mencegah file yang "
    ...
    b.p("Import saldo awal stok (X20) bersifat one-shot: setelah berhasil, penanda "
    ...
    b.bullets([
        ...
        ("Pelacakan. ",
         "UUID job disimpan pada log (job_uuid) sehingga status dapat ditelusuri dari UI."),
        ("Degradasi anggun. ",
    ...
    b.table(["File Type", "Status", "Perilaku Teknis"], [
        ["x101 — Material Master", "FULL",
        ...
        ["x70d — Tender Detail", "Phase-5 gated",
         "pos.payment, di-join ke order X24 berdasarkan kunci (store, date, register, "
         "transnum)."],
        ["x70t / x31 / x32p / store_master", "STAGED",
    ...
    b.p("Log mengikuti state machine queued → running → imported/partial/failed. Executor "
    ...
    b.p("Enam profil di-seed sebagai data (noupdate=1) dengan namespace 'levis'; penyesuaian "
        "per tenant dilakukan dengan mengubah record, bukan kode.")
    b.table(["Profil (code)", "Format", "Data Mulai Baris", "Kolom Kunci (column_map_json)"], [
        ["levis_x101", "xlsx", "3",
        ...
        ["levis_x20", "csv (utf-8-sig)", "2",
         "store_code(16), sap_store_code(17), store_name(18), item_id(19), ean(23), "
         "onhand_qty(29)."],
        ["levis_x24", "xlsx", "2",
    ...
    b.p("Cron \"Retail Import: poll SFTP feeds\" memanggil _cron_poll_feeds() yang mem-poll "
    ...
    b.bullets([
        "Tambahkan openpyxl dan paramiko pada odoo/requirements.txt, lalu rebuild image "
        "Odoo.",
    ...
    b.p("Pemantauan operasional menggunakan: (1) daftar Retail Import Log — status, "
...
def build_charter():
    ...
    b.p("PT EBR sebagai entitas retail Levi's membutuhkan sistem pembukuan dan pelaporan "
    ...
    b.table(["No", "Tujuan", "Kriteria Keberhasilan (Terukur)"], [
        ["1", "Pembukuan EBR berjalan di Odoo",
         "PO EBR→SES, vendor bill, dan pembayaran tercatat di Odoo; tutup buku bulanan "
         "dapat dihasilkan dari Odoo."],
        ["2", "Master data lengkap dan valid",
         "±14.885 artikel / ±159.658 SKU termuat; CoA sesuai file EBR; 24 warehouse "
         "terbentuk; verifikasi count lolos."],
        ["3", "Mirroring operasional toko berjalan",
         "GR, penjualan, tender, dan mutasi stok harian termuat H+1 dengan log audit "
         "lengkap dan nol duplikasi (dedup SHA256 aktif)."],
        ["4", "Otomasi Phase 2",
         "Feed SFTP aktif untuk seluruh jenis file rutin; tanpa unggah manual pada steady "
         "state."],
        ["5", "Rekonsiliasi tender",
         "Settlement tender (X70T) terekonsiliasi terhadap penerimaan bank per acquirer "
         "setiap periode."],
    ], widths=[1.0, 4.6, 10.6])

    b.h("3. Ruang Lingkup", 1)
    b.p("In-scope: implementasi Odoo PT EBR (procurement ke SES, akuntansi, persediaan, "
    ...
    b.bullets([
        ...
        "Dokumen: FSD, TSD, Project Charter, Business Workflow, runbook onboarding, "
        "materi pelatihan pengguna.",
    ])

    b.h("5. Milestone dan Tahapan", 1)
    b.p("Tanggal target diisi saat penjadwalan bersama; urutan tahapan dan dependensinya "
        "adalah sebagai berikut.")
    b.table(["Milestone", "Tahapan", "Keluaran Utama", "Dependensi"], [
        ["M0", "Provisioning tenant (Phase 0)",
         "Database levis + pack retail terpasang.", "—"],
        ["M1", "Konfigurasi dasar & master data (Phase 1)",
         "Company, CoA, bank/journals, pajak, vendor SES, produk X101, 24 warehouse.",
         "M0; file X101 dan CoA (sudah lengkap)."],
        ["M2", "Saldo awal stok (Phase 2 data)",
         "X20 termuat one-shot pada tanggal cut-over.",
         "M1; ekstrak X20 seluruh toko dari pelanggan."],
        ["M3", "Go-live procurement",
         "PO EBR→SES, GR mirror, vendor bill, pembayaran berjalan di produksi.",
         "M1; kode toko lengkap untuk crosswalk."],
        ["M4", "Keputusan dan go-live penjualan (Phase 5)",
         "Posting X24/X70D diaktifkan setelah keputusan representasi POS, histori, dan tax "
         "mapping.", "M3; workshop Finance EBR dan pajak."],
        ["M5", "Otomasi SFTP (Phase 6)",
         "Feed SFTP aktif, cron poller berjalan, unggah manual dihentikan.",
         "M3; akses server FTP/SFTP Erajaya."],
        ["M6", "Stabilisasi dan serah terima",
         "Rekonsiliasi 1 siklus bulanan penuh; handover ke operasional.", "M4, M5."],
    ], widths=[1.6, 4.6, 6.0, 4.0])

    b.h("6. Organisasi Proyek", 1)
    b.table(["Peran", "Pihak", "Tanggung Jawab Utama"], [
        ["Project Sponsor", "Business Owner Levi's Retail (Erajaya)",
         "Keputusan bisnis, pendanaan, eskalasi akhir."],
        ["Project Manager", "Erajaya",
         "Perencanaan, koordinasi lintas pihak, pelaporan status, manajemen risiko."],
        ["Functional Consultant", "Erajaya",
         "Konfigurasi Odoo, mapping proses, UAT, pelatihan."],
        ["Technical Lead / Developer", "Erajaya",
         "Modul custom_retail_import, profil import, feed SFTP, deployment."],
        ["Finance & Accounting", "PT EBR",
         "CoA, kebijakan pembukuan, keputusan Phase 5, rekonsiliasi, UAT keuangan."],
        ["IT / Master Data", "PT SES",
         "Penyediaan ekstrak SAP/POS, kode toko, koordinasi dengan principal."],
        ["Principal Liaison", "Levi's Principal",
         "Akses report POS, jadwal drop file, perubahan struktur report."],
    ], widths=[3.6, 4.4, 8.2])

    b.h("7. Risiko Utama dan Mitigasi", 1)
    b.table(["No", "Risiko", "Dampak", "Mitigasi"], [
        ["1", "Data transaksi baru tersedia untuk 1 dari 24 toko (sampel).",
         "Go-live penjualan tertunda.",
         "Eskalasi permintaan ekstrak penuh / aktivasi feed SFTP lebih awal; go-live "
         "bertahap per toko."],
        ["2", "Kode toko SAP belum tersedia untuk 23 toko.",
        ...
        ["3", "Keputusan Phase 5 (representasi POS, histori, tax mapping) tertunda.",
         "Posting penjualan dan tender tertahan gating.",
         "Workshop keputusan dijadwalkan sebelum M4 dengan opsi default yang "
         "direkomendasikan."],
        ["4", "Struktur report principal berubah tanpa pemberitahuan.",
         "Parsing gagal atau salah kolom.",
         "Profil import berbasis data (ubah mapping tanpa kode); dry-run preview; notifikasi "
         "kegagalan parse."],
        ["5", "Kredensial SFTP tersimpan tanpa enkripsi at-rest.",
         "Risiko kebocoran kredensial.",
         "Preferensi autentikasi private key; pembatasan akses ir.config_parameter; "
         "roadmap enkripsi platform."],
        ["6", "Volume X101 besar (±159 ribu SKU) membebani import.",
         "Timeout / beban server.",
         "Eksekusi asinkron queue_job, parsing streaming, commit berbatch (teruji ±30 "
         "menit)."],
    ], widths=[1.0, 4.6, 3.6, 7.0])

    b.h("8. Asumsi dan Batasan", 1)
    b.bullets([
        "Odoo EBR adalah sistem pembukuan; POS principal dan SAP SES tetap menjadi sumber "
        "transaksi.",
    ...
    b.p("Perubahan ruang lingkup setelah charter disetujui diajukan melalui change request "
...
def build_workflow():
    b = Builder()
    b.cover("BUSINESS WORKFLOW",
            "Business Workflow — Proses Bisnis Retail Levi's pada Odoo",
            SUBTITLE, "BW/ERAJAYA/LEVIS-ODOO/2026/004")
    b.approval(APPROVAL_INTRO, APPROVAL_ROWS)

    b.h("1. Pendahuluan", 1)
    b.p("Dokumen ini menjabarkan alur kerja bisnis (business workflow) operasional retail "
        ...
        "pada docs/projects/levis/business-process-flow.html.")

    b.h("2. Peran dan Tanggung Jawab (RACI Ringkas)", 1)
    b.table(["Aktivitas", "Toko", "SES (SAP)", "EBR Finance (Odoo)", "Erajaya IT"], [
    ...
    b.p("R = Responsible (pelaksana), A = Accountable (penanggung jawab), C = Consulted, "
        "I = Informed.", italic=True)

    b.h("3. Workflow Utama", 1)

    b.h("3.1 WF-A — Pengadaan SES kepada Principal (Konteks)", 2)
    b.p("Alur ini sepenuhnya berjalan pada SAP SES dan menjadi konteks hulu: SES "
    ...
    b.table(["Langkah", "Aktor", "Aktivitas", "Input", "Output"], [
        ["B1", "EBR Purchasing (Odoo)",
         "Membuat PO ke vendor PT SES; approval berjenjang via approval engine sesuai "
         "matriks otorisasi.", "Rencana kebutuhan toko", "PO approved (Odoo)"],
        ["B2", "SES (SAP)",
         "Menerima PO, membentuk SO dan Delivery, mengirim barang DC → toko.",
         "PO EBR", "Surat jalan, barang tiba di toko"],
        ["B3", "Toko (POS principal)",
         "Memeriksa fisik barang dan melakukan GR pada POS; selisih dicatat sebagai "
         "discrepancy ke SES.", "Surat jalan + fisik barang", "GR POS; stok toko bertambah"],
        ["B4", "EBR Finance (Odoo)",
         "Menerima mirror GR (file X32P/GR) → memvalidasi receipt atas PO; stok dan akrual "
         "hutang tercatat.", "File mirror GR", "Receipt tervalidasi di Odoo"],
        ["B5", "EBR Finance (Odoo)",
         "Mencatat vendor bill SES, mencocokkan PO–GR–Bill, menjadwalkan dan membayar.",
         "Invoice SES", "Bill posted; pembayaran terlaksana"],
    ], widths=[1.4, 3.4, 5.6, 2.9, 2.9])

    b.h("3.3 WF-C — Siklus Harian Toko (EOD) dan Mirroring", 2)
    b.table(["Langkah", "Aktor", "Aktivitas", "Waktu"], [
        ["C1", "Toko", "Transaksi sepanjang hari pada POS principal: penjualan, retur, "
         "pembayaran multi-tender; GR/transfer bila ada kiriman.", "Sepanjang hari"],
        ["C2", "Toko / Principal", "Tutup kasir; report harian terbentuk di sistem "
         "principal (X24DN, X70D/X70T, X32P, X20).", "EOD"],
        ["C3", "Ops (Phase 1) / Sistem (Phase 2)",
        ...
        ["C5", "EBR Finance",
         ...
         "rekonsiliasi settlement tender terhadap mutasi bank per acquirer.", "H+1"],
    ...
    b.bullets([
        ...
        ("Klaim promosi. ",
         "X31 (discount journal) direkap sebagai dasar klaim promosi kepada principal/SES."),
        ("Tutup buku. ",
    ...
    b.table(["Kode", "Pengecualian", "Penanganan", "Peran"], [
        ["E1", "File duplikat diunggah (hash SHA256 sama dengan import sukses/berjalan "
         "sebelumnya).",
        ...
        ["E2", "File gagal parse (struktur kolom berubah, encoding salah).",
         ...
         "failed tidak memblokir dedup.", "Erajaya IT + EBR Finance"],
        ["E3", "Import partial (sebagian baris error).",
         ...
         "baris yang sudah masuk ter-update, bukan terduplikasi.", "EBR Finance"],
        ["E4", "Feed SFTP error (koneksi/kredensial/direktori).",
         ...
         "Erajaya IT"],
        ["E5", "Selisih GR vs PO (qty kurang/lebih, barang rusak).",
        ...
        ["E6", "Selisih rekonsiliasi tender vs bank.",
         "Identifikasi per acquirer per hari; selisih fee dibukukan; selisih abnormal "
         "dieskalasi ke acquirer/principal.", "EBR Finance"],
        ["E7", "Saldo awal X20 dijalankan ulang.",
         "Sistem menolak otomatis (guard one-shot ir.config_parameter); pengulangan sah "
         "memerlukan keputusan formal dan reset penanda oleh IT.", "Erajaya IT"],
    ...
    b.bullets([
        ...
        ("Approval berjenjang. ",
         "PO EBR melewati approval engine sebelum dikirim ke SES."),
    ...
    b.table(["KPI", "Target", "Sumber Pengukuran"], [
        ["Ketepatan waktu mirroring harian", "Data EOD termuat ≤ H+1 jam 10.00",
         "retail.import.log (imported_at vs tanggal transaksi)"],
        ["Tingkat keberhasilan import", "≥ 99% log berstatus imported (bukan failed)",
         "Statistik log per bulan"],
        ["Duplikasi transaksi", "0 kejadian", "Audit dedup + rekonsiliasi"],
        ["Selisih rekonsiliasi tender", "≤ ambang fee acquirer yang disepakati",
         "Rekonsiliasi bulanan"],
        ["Penyelesaian exception", "E1–E7 selesai ≤ 2 hari kerja", "Tindak lanjut log/feed"],
    ], widths=[5.4, 5.4, 5.4])

    b.p("Dengan disetujuinya dokumen ini, alur kerja pada Bab 3–4 menjadi acuan operasional "
