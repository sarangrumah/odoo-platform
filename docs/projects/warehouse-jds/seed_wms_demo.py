...
def seed_putaway_suggestions():
    ...
    pick = env["stock.picking"].create(
        {
            ...
            "origin": "PO Levi's SS26 Inbound",
        }
    )
    lines_map = []
    for prod, qty, loc in [
        (products[0], 48, bins["A-01-01"]),
        (products[1], 12, bins["A-02-01"]),
        (products[2], 30, bins["A-01-02"]),
        (products[5], 60, bins["B-03-01"]),
    ...
    pick.action_assign()
    sug_meta = [
        (96, "Fixed location", "applied"),
        (95, "ABC velocity (A)", "applied"),
        (88, "Nearest empty: A-01-02", "pending"),
        (72, "Volume fit: 6.2 m3 free", "pending"),
    ...
    for (prod, loc, qty), (score, reason, status) in zip(lines_map, sug_meta):
        ...
        PSug.create(
            {
                "name": "Putaway " + prod.name,
                "move_line_id": ml.id,
                "picking_id": pick.id,
                "suggested_location_id": loc.id,
                "original_dest_location_id": stock_loc.id,
                "rule_id": rule0.id if rule0 else False,
                "score": score,
                "reason": reason,
                "status": status,
                "company_id": company.id,
...
def seed_hht():
    ...
    if not Dev.search([]):
        ...
        for nm, sn, mdl, cidr in [
            ...
            d = Dev.create(
                {"name": nm, "device_id": sn, "model": mdl, "enabled": True, "allowed_cidrs": cidr, "user_id": admin.id}
            )
            d.write(
                {
                    "last_seen_at": now,
    ...
    pick = env["stock.picking"].create(
        {
            ...
            "location_dest_id": zoneA.id,
            "origin": "Replenishment Wave R-0042",
        }
    )
    bs = BSess.create(
        {"name": "Scan - Wave R-0042", "picking_id": pick.id, "operator_id": admin.id, "state": "scanning"}
    )
...
def seed_quality():
    ...
    qp1 = QP.create(
        {
            "name": "Incoming - Stitching & Fabric Grade",
            "operation": "incoming",
    ...
    qp2 = QP.create(
        {
            "name": "Incoming - Measure Inseam (cm)",
            "operation": "incoming",
            "check_kind": "measure",
            "frequency": "random",
    ...
    QP.create(
        {
            "name": "Outgoing - Final Pack Audit",
            "operation": "outgoing",
            "check_kind": "visual",
            "frequency": "periodic",
    ...
    QC.create(
        {
            ...
            "state": "pass",
            "user_id": admin.id,
            "note": "Stitching OK, fabric grade A.",
    ...
    c_fail = QC.create(
        {
            ...
            "user_id": admin.id,
            "measure_value": 78.5,
            "note": "Inseam below tolerance (78.5 < 80).",
            "performed_at": fields.Datetime.now(),
        }
    )
    alert = QA.create(
        {
            "name": "NCR - Inseam out of tolerance",
            "check_id": c_fail.id,
            "product_id": products[2].id,
            "state": "investigating",
        }
    ...
    CAPA.create(
        {
            ...
            "deadline": fields.Date.context_today(env.user),
            "state": "in_progress",
        }
    )
    note(
        "quality: %d points, %d checks, %d alerts, %d capas"
