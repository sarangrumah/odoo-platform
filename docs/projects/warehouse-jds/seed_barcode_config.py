...
log = []
def note(m): log.append(m); print("[seedbc]", m)
def safe(label, fn):
    try:
...
def seed_labels():
    ...
    L.create({"name": "Product Label (Zebra 2x1)", "paper_format": "zebra_2x1",
              "output_mode": "zpl", "template_source": zpl_prod,
              "applies_to": prod_model.id if prod_model else False, "company_id": company.id})
    L.create({"name": "Pallet / SSCC (Zebra 4x6)", "paper_format": "zebra_4x6",
              "output_mode": "zpl", "template_source": zpl_pallet, "company_id": company.id})
    L.create({"name": "Bin Location (Thermal 50x30)", "paper_format": "thermal_50x30",
              "output_mode": "escpos", "template_source": "[BIN] {{location.complete_name}}",
              "applies_to": loc_model.id if loc_model else False, "company_id": company.id})
...
def seed_printers():
    ...
    P.create({"name": "Receiving Dock - Zebra ZD420", "printer_type": "zebra_network",
              "host": "10.20.0.50", "port": 9100,
              "label_template_id": prod_lbl.id if prod_lbl else False, "company_id": company.id})
    P.create({"name": "Pack Bench - Zebra ZT411", "printer_type": "zebra_network",
              "host": "10.20.0.51", "port": 9100,
              "label_template_id": pallet_lbl.id if pallet_lbl else False, "company_id": company.id})
    P.create({"name": "Office - CUPS Queue", "printer_type": "cups",
              "cups_queue": "HP_LaserJet_Office", "company_id": company.id})
