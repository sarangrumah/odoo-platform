...
Usage:  python docs/projects/finance-portal/build_architecture_docx.py
"""
import io
import os
...
def _box(ax, x, y, w, h, label, sublabel="", color=M_ODOO,
         textcolor=M_WHITE, fontsize=9, radius=0.25, zorder=3):
    box = FancyBboxPatch((x, y), w, h,
                         boxstyle=f"round,pad=0,rounding_size={radius}",
                         facecolor=color, edgecolor=M_WHITE,
...
def _arrow(ax, x1, y1, x2, y2, label="", color="#444", lw=1.5, zorder=2):
    ...
    if label:
        ...
        ax.text(mx, my + 0.15, label, ha="center", va="bottom",
                fontsize=7.5, color=color, zorder=zorder + 1,
                bbox=dict(boxstyle="round,pad=0.2", facecolor=M_WHITE,
...
def _legend(ax, items, x=0.3, y=0.35, fs=8):
    for i, (color, label) in enumerate(items):
        ...
        ax.add_patch(FancyBboxPatch((px, y), 0.35, 0.25,
                                    boxstyle="round,pad=0,rounding_size=0.05",
                                    facecolor=color, edgecolor="none"))
        ax.text(px + 0.45, y + 0.12, label, va="center",
...
def diagram_system_context():
    ...
    # Title
    ax.text(8, 9.6, "System Context — Finance Portal", ha="center",
            fontsize=14, fontweight="bold", color=C_INK.rgb if hasattr(C_INK, 'rgb') else "#1A2332")
    ...
    ax.text(6.0, 8.5, "Finance Portal DB  (BARU, TERPISAH)",
            ha="center", fontsize=9, fontweight="bold", color=M_NEW,
    ...
    _box(ax, 3.2, 3.0, 2.4, 0.38, "Existing Odoo DB  (tidak disentuh)",
         color=M_WARN, fontsize=7.5)

    # ── Bridge ──
    _box(ax, 9.6, 5.5, 2.8, 2.2,
         "SAP Bridge",
         "FastAPI · Kafka Consumer/Producer\nRedis Cache",
         color=M_BRIDGE, fontsize=8.5)
...
def diagram_kafka_integration():
    fig, ax = _fig(16, 9)
    ax.set_facecolor(M_BG)
    ax.text(8, 8.6, "Integrasi Kafka-Only — Bridge sebagai Cache Layer",
            ha="center", fontsize=13, fontweight="bold", color="#1A2332")

    # Rule banner
    rect = FancyBboxPatch((2.5, 7.9), 11.0, 0.55,
                          boxstyle="round,pad=0,rounding_size=0.1",
                          facecolor="#FDEBD0", edgecolor="#E59866", lw=1.5)
    ax.add_patch(rect)
    ax.text(8.0, 8.17,
            "ATURAN: Bridge TIDAK BOLEH call SAP REST langsung — semua via Kafka topic",
            ha="center", va="center", fontsize=9, color="#784212", fontweight="bold")
    ...
    for y, label, seg1, seg2, seg3 in rows:
        ax.text(0.1, y + 0.12, label, va="center", fontsize=7.5,
    ...
    _box(ax, 5.3, 0.8, 2.6, 1.1, "Redis Cache",
         "pr · po_gr · vendor\nbudget · employee · travel",
         color="#C0392B", fontsize=8)
    ax.annotate("", xy=(6.6, 1.9), xytext=(6.6, 3.1),
...
def diagram_ocr_pipeline():
    fig, ax = _fig(13, 11)
    ax.set_facecolor(M_BG)
    ax.text(6.5, 10.6, "OCR Pipeline — Scanned PDF (primary path)",
            ha="center", fontsize=12, fontweight="bold", color="#1A2332")
    ...
    def diamond(ax, cx, cy, half_w, half_h, text, color=M_KAFKA):
        ...
        ax.text(cx, cy, text, ha="center", va="center", fontsize=8,
                fontweight="bold", color=M_WHITE, zorder=4)
    ...
    ax.text(6.65, 7.05, "Tidak\n(primary)", fontsize=8,
            color=M_ODOO, fontweight="bold")
    process(4.8, 6.3, 3.4, 0.75, "pdf2image → 300 DPI PNG", color=M_ODOO)
    arr(6.5, 6.3, 6.5, 5.8)
    process(4.8, 5.1, 3.4, 1.0,
            "OpenCV Pre-processing\nGrayscale · Denoise\nDeskew · Binarize · Upscale",
            color=M_ODOO)
    ...
    # Merge → regex parser
    process(4.2, 2.6, 4.6, 0.7,
            "Regex Parser: invoice no · date · NPWP\ntotal · PPN · PPh · PO/DO ref",
            color=M_BRIDGE)
...
def diagram_multicompany():
    fig, ax = _fig(14, 8)
    ax.set_facecolor(M_BG)
    ax.text(7.0, 7.6, "Multi-Company Architecture — Single Finance Portal DB",
            ha="center", fontsize=12, fontweight="bold", color="#1A2332")

    # DB container
    db_box = FancyBboxPatch((0.3, 0.5), 13.4, 6.8,
                            boxstyle="round,pad=0,rounding_size=0.3",
                            facecolor="#EAF2FB", edgecolor="#1168BD", lw=2, zorder=1)
    ax.add_patch(db_box)
    ax.text(7.0, 7.1, "Finance Portal DB  (PostgreSQL — single database)",
            ha="center", fontsize=9, color=M_ODOO, fontweight="bold")

    # Finance Group Manager (top center)
    _box(ax, 5.0, 5.9, 4.0, 0.85,
         "Finance Group Manager",
         "company_ids = [PT-A, PT-B, PT-C ...]\nDashboard: semua PT",
         color=M_HOLDING, fontsize=8)
    ...
    ax.text(7.0, 2.78, "Shared across all companies",
...
def diagram_state_machine():
    ...
    fig, ax = _fig(15, 7)
    ax.set_facecolor(M_BG)
    ax.text(7.5, 6.6, "Document Lifecycle — State Machine (semua tipe dokumen)",
            ha="center", fontsize=12, fontweight="bold", color="#1A2332")
    ...
    colors = {
        "draft": "#7F8C8D", "submitted": M_ODOO, "tax_review": "#D4AC0D",
        "fin_review": "#CA6F1E", "approved": M_BRIDGE,
        "pushed": "#1A5276", "posted": "#1F618D", "paid": "#117A65",
        "rejected": M_WARN, "cancelled": "#95A5A6",
    }
    labels = {
        "draft": "Draft", "submitted": "Submitted", "tax_review": "Tax\nReview",
    ...
    def sa(frm, to, lbl="", offset=(0, 0)):
        fx, fy = states[frm]; tx, ty = states[to]
        x1 = fx + W / 2 + offset[0]; y1 = fy + H / 2
        x2 = tx + W / 2 + offset[0]; y2 = ty + H / 2
    ...
    for a, b, l in [
        ...
        fx, fy = states[a]; tx, ty = states[b]
        _arrow(ax, fx + W / 2, fy, tx + W / 2, ty + H,
               l, color=M_BRIDGE, lw=1.8)

    # Reject paths
    _arrow(ax, states["tax_review"][0] + W / 2, states["tax_review"][1],
           states["rejected"][0] + W / 2, states["rejected"][1] + H,
           "Reject", color=M_WARN, lw=1.5)
    _arrow(ax, states["fin_review"][0] + W / 2, states["fin_review"][1],
           states["rejected"][0] + W / 2, states["rejected"][1] + H,
           "Reject", color=M_WARN, lw=1.5)

    # Rejected → draft
    _arrow(ax, states["rejected"][0], states["rejected"][1] + H / 2,
           states["draft"][0] + W, states["draft"][1] + H / 2,
           "Revise", color="#E67E22", lw=1.3)

    # Cancelled
    ax.text(1.5, 2.95, "cancel()\n(sebelum approved)", fontsize=7.5,
            color="#7F8C8D", ha="center", style="italic")
    _arrow(ax, states["submitted"][0] + W / 2, states["submitted"][1],
           states["cancelled"][0] + W / 2, states["cancelled"][1] + H,
           "", color="#95A5A6", lw=1.2)
...
class Builder:
    ...
    def p(self, text, bold=False, italic=False, align=WD_ALIGN_PARAGRAPH.JUSTIFY,
          size=None, color=None, space_after=5):
    ...
    def bullets(self, items):
        for it in items:
            ...
            if isinstance(it, tuple):
                r = par.add_run(it[0]); r.bold = True; r.font.size = Pt(10)
                r2 = par.add_run(it[1]); r2.font.size = Pt(10)
            else:
                r = par.add_run(str(it)); r.font.size = Pt(10)
    ...
    def table(self, headers, rows, widths=None, total_rows=None,
              highlight_rows=None, head_bg=HEAD_BG, cell_size=9.5):
        ...
        for ridx, row in enumerate(rows):
            ...
            for i, val in enumerate(row):
                ...
                run.font.size = Pt(cell_size); run.bold = is_total
                if is_total:   self._shade(cells[i], TOTAL_BG)
                elif row_bg:   self._shade(cells[i], row_bg)
        if widths:
    ...
    def cover(self, kind, title, subtitle, attrs):
        ...
        self.p(kind, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER,
               size=22, color=C_ACCENT, space_after=2)
        self.doc.add_paragraph()
...
def build():
    ...
    b.table(
        ...
        [
            ["ADR-01", "Peran Odoo", "System of Engagement (tanpa GL posting)",
             "SAP tetap System of Record"],
            ["ADR-02", "Database", "DB BARU terpisah dari existing Odoo DB",
             "Isolasi lifecycle; tidak ganggu operasional existing"],
            ["ADR-03", "Multi-PT Erajaya", "Single Finance Portal DB + res.company per PT",
             "Consolidated dashboard; shared vendor master; satu SSO"],
            ["ADR-04", "Integrasi SAP", "Kafka-only — Bridge TIDAK boleh call SAP REST langsung",
             "Constraint historis: direct SAP connection tidak disarankan"],
            ["ADR-05", "Bridge pattern", "Kafka consumer/producer + Redis cache + HMAC REST relay",
             "Odoo tidak kenal Kafka; Bridge satu-satunya integration point"],
            ["ADR-06", "SSO karyawan", "Keycloak OIDC via auth_oauth",
             "HRIS maintain employee identity"],
            ["ADR-07", "Login vendor", "Odoo native portal: self-registration + invite",
             "HRIS tidak maintain vendor; vendor bukan employee"],
            ["ADR-08", "OCR engine", "Tesseract 4 LSTM (primary) + pdfplumber (fallback)",
             "CPU-only (VM tanpa GPU); scanned PDF sebagai format utama"],
            ["ADR-09", "Travel dinas", "Read-only mirror dari HRIS",
             "HRIS owns travel engine"],
            ["ADR-10", "Async push", "queue_job with_delay()",
             "Hindari timeout; decouple dari SAP latency"],
        ],
        widths=[1.5, 3.5, 5, 7],
        cell_size=8.5,
    )
    ...
    b.image(diagram_system_context(), width_cm=16.5,
    ...
    b.image(diagram_kafka_integration(), width_cm=16.5,
    ...
    b.note("Baris hijau = outbound (Odoo -> SAP). Nama topik, retention, dan partition count disepakati tim Kafka klien.")
    ...
    b.bullets([
        ...
        ("Cache MISS (PR baru): ", "Bridge return {found: false, retry_after: 30}. "
         "UI Odoo tampilkan: 'PR sedang disinkronisasi dari SAP, coba lagi dalam 30 detik'"),
        ("Implikasi ke SAP team: ", "SAP harus produce PR events secara event-driven (tidak batch) "
         "agar cache selalu fresh. PO/GR sama."),
    ])

    # ── 5. Document Lifecycle ───────────────────────────────────────────────
    b.doc.add_page_break()
    b.h("5. Document Lifecycle", 1)
    b.image(diagram_state_machine(), width_cm=16.5,
    ...
    b.image(diagram_ocr_pipeline(), width_cm=14.0,
    ...
    b.image(diagram_multicompany(), width_cm=15.5,
    ...
    b.table(
        ...
        [
            ["Karyawan PT A", "company_ids = [PT-A]",
             "Submit → company_id = PT-A; company switcher tidak tampil (single company)"],
            ["Finance Officer PT B", "company_ids = [PT-B]",
             "Hanya lihat dokumen PT-B"],
            ["Finance Group Manager", "company_ids = [PT-A, PT-B, PT-C ...]",
             "Dashboard 'All Companies' aggregate semua PT; bisa drill-down per PT"],
            ["Vendor 'Supplier X'", "res.partner (shared, tidak company-specific)",
    ...
    b.table(
        ...
        [
            ["Employee Master", "hris.to.portal.employee-master",
            ...
            ["Travel Request", "hris.to.portal.travel-request",
    ...
    b.table(
        ...
        [
            ["1",  "Supplier/Vendor",        "sap.to.portal.vendor-master",    "Daily+change", "TBD", "Vendor Invoice tidak bisa divalidasi"],
            ...
            ["8",  "PO/GR Master (event!)",  "sap.to.portal.po-gr-master",     "Event-driven", "TBD", "Vendor Invoice PO tidak bisa validasi"],
            ...
            ["11", "Employee Master",        "hris.to.portal.employee-master", "Daily+change", "TBD", "User provisioning manual"],
            ["12", "Travel Request",         "hris.to.portal.travel-request",  "Event",        "TBD", "Travel Settlement tidak bisa dibuat"],
        ],
    ...
    b.table(
        ...
        [
            ["nik", "hr.employee", "Spesifik (Pasal 4)",
             "pdp_class='specific'; masking view non-privileged"],
            ["bank_account_no", "res.partner, hr.employee", "Spesifik",
             "Masking 4 digit terakhir; full hanya group_finance_officer+"],
            ["npwp", "res.partner", "Spesifik",
             "Masking 6 digit tengah; full hanya group_finance_officer+"],
            ["Transisi state", "finance.document.mixin", "—",
             "pdp.audited.mixin → INSERT INTO pdp.audit_log (raw SQL)"],
            ["Log adapter", "custom.adapter.call.log", "—",
