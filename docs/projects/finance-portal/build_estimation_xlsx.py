...
Usage : python docs/projects/finance-portal/build_estimation_xlsx.py
Output: docs/projects/finance-portal/Project-Estimation-Finance-Portal-Odoo.xlsx
"""
...
def w(ws, r, c, v, bg=WHITE, bold=False, color=NAVY, sz=10, h="left", wrap=True, italic=False):
...
def merge_title(ws, r, c1, c2, v, bg=NAVY, sz=14, bold=True, color=WHITE, h="center", italic=False):
...
def merge_w(ws, r, c1, c2, v, bg=WHITE, bold=False, color=NAVY, sz=10, h="left", italic=False):
    ws.merge_cells(start_row=r, start_column=c1, end_row=r, end_column=c2)
    cell = ws.cell(row=r, column=c1, value=v)
    cell.fill = fill(bg)
    cell.font = fnt(bold=bold, color=color, size=sz, italic=italic)
...
def build_ringkasan(wb):
    ...
    cards = [
        (2, 3, "Total Proyek\n(incl. PMO + Kontingensi 15%)", 462, "Semua 7 fase · semua role"),
        (4, 5, "Total Non-PMO\n(dengan Kontingensi 15%)", 408, "BA + Dev + QA · semua 7 fase"),
        (6, 7, "Build Phase Non-PMO\n(tanpa Kontingensi)", 202, "BA + Dev + QA · Fase Build saja"),
    ]
    ...
    phases = [
        ("1", "Requirement & Analysis", 8, 22, 3, 4, 37, "FSD draft, kontrak integrasi final"),
        ("2", "Design (FSD/TSD)", 3, 8, 10, 6, 27, "TSD, data model sign-off, mockup"),
        (
            "3",
            "Build (lihat sheet Build Detail)",
            18,
            35,
            123,
            44,
        ...
        ("4", "SIT", 5, 6, 12, 16, 39, "E2E pass — gated: konektor SAP/Kafka/HRIS ready"),
        ("5", "UAT", 5, 12, 12, 8, 37, "UAT sign-off user"),
        ("6", "Cutover & Go-Live", 4, 5, 8, 3, 20, "Parallel run, cutover day"),
        ("7", "Hypercare", 4, 5, 9, 3, 21, "Stabilisasi 3 minggu, handover support"),
    ]
    ...
    r = 17
    sub(ws, r, 2, "")
    sub(ws, r, 3, "Subtotal (tanpa kontingensi)")
    for col, v in [(4, 47), (5, 93), (6, 177), (7, 84), (8, 401)]:
        sub(ws, r, col, v)
    w(ws, r, 9, "", bg=LILA)

    r = 18
    for col, v in [(2, ""), (3, "+ Kontingensi 15%"), (4, 7), (5, 14), (6, 27), (7, 13), (8, 61)]:
        w(ws, r, col, v, bg=LILA, bold=True, h="center" if isinstance(v, int) else "left")
    w(ws, r, 9, "", bg=LILA)

    r = 19
    tot(ws, r, 2, "")
    tot(ws, r, 3, "TOTAL (dengan kontingensi)")
    for col, v in [(4, 54), (5, 107), (6, 204), (7, 97), (8, 462)]:
        tot(ws, r, col, v)
    w(ws, r, 9, "Grand total termasuk PMO", bg=LILA2, bold=True)

    r = 20
    tot(ws, r, 2, "")
    tot(ws, r, 3, "Total Non-PMO  =  462 − 54")
    for col, v in [(4, "—"), (5, 107), (6, 204), (7, 97), (8, 408)]:
        tot(ws, r, col, v)
    w(ws, r, 9, "Angka ini setara spreadsheet klien (tanpa PMO)", bg=LILA2, italic=True, sz=9)
...
def build_semua_fase(wb):
    ...
    merge_title(ws, 1, 2, 9, "Estimasi Mandays — Semua Fase  (Total Non-PMO = 408 md  ·  incl. PMO = 462 md)", sz=13)

    for col, lbl in enumerate(
        ["No", "Fase / Aktivitas", "PMO", "BA", "Dev", "QA", "Total", "% dari total", "Keterangan"], 1
    ):
        hdr(ws, 2, col, lbl)
    ws.row_dimensions[2].height = 20

    phases = [
        ("1", "Requirement & Analysis", 8, 22, 3, 4, 37, "FSD draft, stakeholder interview, kontrak integrasi final"),
        ("2", "Design (FSD/TSD)", 3, 8, 10, 6, 27, "TSD & data model sign-off, UI mockup, JSON payload spec"),
        (
            "3",
            "Build / Development",
            18,
        ...
        ("4", "SIT", 5, 6, 12, 16, 39, "Gated: konektor SAP/Kafka/HRIS ready; E2E pass"),
        ("5", "UAT", 5, 12, 12, 8, 37, "UAT sign-off dari user"),
        ("6", "Cutover & Go-Live", 4, 5, 8, 3, 20, "Data migration, parallel run, cutover day"),
        ("7", "Hypercare", 4, 5, 9, 3, 21, "Stabilisasi 3 minggu, handover ke support"),
    ]
    for i, (no, fase, pmo, ba, dev, qa, ttl, note) in enumerate(phases):
        r = 3 + i
        bg = ALT if i % 2 else WHITE
    ...
    r = 10
    sub(ws, r, 1, "")
    sub(ws, r, 2, "Subtotal (tanpa kontingensi)")
    for col, v in [(3, 47), (4, 93), (5, 177), (6, 84), (7, 401), (8, "100%")]:
        sub(ws, r, col, v)
    w(ws, r, 9, "", bg=LILA)

    r = 11
    for col, v in [(1, ""), (2, "+ Kontingensi 15%"), (3, 7), (4, 14), (5, 27), (6, 13), (7, 61), (8, "15%")]:
        w(ws, r, col, v, bg=LILA, bold=True, h="center" if isinstance(v, (int, float)) or "%" in str(v) else "left")
    w(ws, r, 9, "", bg=LILA)

    r = 12
    tot(ws, r, 1, "")
    tot(ws, r, 2, "TOTAL (dengan kontingensi)")
    for col, v in [(3, 54), (4, 107), (5, 204), (6, 97), (7, 462), (8, "115%")]:
        tot(ws, r, col, v)
    w(ws, r, 9, "Grand total — termasuk PMO", bg=LILA2, bold=True)

    r = 13
    tot(ws, r, 1, "")
    tot(ws, r, 2, "Total Non-PMO  =  462 − 54 PMO")
    for col, v in [(3, "—"), (4, 107), (5, 204), (6, 97), (7, 408), (8, "")]:
        tot(ws, r, col, v)
...
FEATURE_ROWS = [
    ...
    (
        ...
        "C",
        "CA Realization",
        "Realization CA – Create/Edit form",
    ...
    (
        ...
        "C",
        "CA Realization",
        "Approval Realization Tax→Finance",
    ...
    (
        ...
        "C",
        "CA Realization",
        "Terima status payment dari SAP",
    ...
    (
        ...
        "E",
        "Invoice PO",
        "Push MIRO posting ke SAP (PO-linked)",
    ...
    (
        ...
        "E",
        "Vendor Portal",
        "Submit invoice + tracking status",
    ...
    ("F.4", "F", "PD Realization", "List & Filter", "List view", "custom_finance_portal", "Internal", "Low", 0, 1, 0.5),
    (
        "F.5",
        "F",
        "PD Realization",
        "Create/Edit form Realization",
    ...
    (
        ...
        "G",
        "Budget Control",
        "Remaining budget display (computed)",
    ...
    (
        ...
        "G",
        "Budget Control",
        "Alert + blocking rule over-budget",
    ...
    (
        ...
        "H",
        "Integration",
        "Adapter base (REST caller + retry)",
    ...
    (
        ...
        "H",
        "Integration",
        "Webhook receiver + HMAC validator",
    ...
    (
        ...
        "J",
        "Configuration",
        "Limitation for Submission master",
    ...
    (
        ...
        "J",
        "Configuration",
        "Sync config (endpoint, topic, token)",
...
def build_detail(wb):
    ...
    merge_title(
        ...
        1,
        12,
        "Build Phase Detail — Finance Portal Odoo  (BA 35 + Dev 123 + QA 44 = 202 mandays, tanpa PMO & kontingensi)",
        sz=12,
    )
    ...
    for row_data in FEATURE_ROWS:
        ...
        if ws_code != cur_ws_code:
            ...
            cell = ws.cell(row=r, column=1, value=f"  {ws_code}.  {ws_name}")
    ...
    r += 1
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=8)
    cell = ws.cell(row=r, column=1, value="GRAND TOTAL FASE BUILD  (tanpa PMO, tanpa kontingensi)")
    cell.fill = fill(LILA2)
    cell.font = fnt(bold=True, color=NAVY, size=11)
    ...
    for cplx, ba, dev, qa in [("Low", 0.5, 1.5, 0.5), ("Medium", 1, 3, 1), ("High", 2, 5, 2)]:
        for col, v in enumerate([cplx, ba, dev, qa, ba + dev + qa], 1):
            w(
                ...
                col,
                v,
                bg=ALT if cplx == "Medium" else WHITE,
                h="center" if isinstance(v, (int, float)) else "left",
            )
        ws.row_dimensions[r].height = 18
        r += 1
