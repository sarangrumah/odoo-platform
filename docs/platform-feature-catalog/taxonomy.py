...
TENANTS = [
    {
        "id": "arkaaim",
        "brand": "ARKA-AIM",
        "legal": "PT Arka Mandiri Nusantara / PT AIM",
        "erajaya": True,
        "dbs": ["prd_arkaaim", "trn_arkaaim"],
        "status": "live",
    ...
    },
    {
        "id": "levis",
        "brand": "Levi's",
        "legal": "PT Era Busana Retailindo (EBR)",
        "erajaya": True,
        "dbs": ["prd_levis_begbal", "rnd_levis"],
        "status": "live",
        "industry": "Retail fashion",
    },
    {
        "id": "ppob",
        "brand": "Eraspace / PPOB-VAS",
        "legal": "PT Erajaya Swasembada Tbk",
        "erajaya": True,
        "dbs": ["rnd_ppob"],
        "status": "development",
    ...
    {
        "id": "efn",
        "brand": "EFN (Erajaya F&B)",
        "legal": "Erajaya Food & Nourishment",
        "erajaya": True,
        "dbs": ["rnd_esb"],
        "status": "pre-pilot",
    ...
    {
        "id": "finportal",
        "brand": "Finance Portal",
        "legal": "Erajaya Group Shared Services",
        "erajaya": True,
        "dbs": [],
        "status": "development",
    ...
    },
    {
        "id": "vaspmo",
        "brand": "VAS PMO",
        "legal": "Erajaya Value-Added Services",
        "erajaya": True,
        "dbs": ["rnd_vas_pmo"],
        "status": "live",
...
ID_LABELS = {
    ...
    "custom_levis_operating_unit": (
        "Migrasi Unit Operasional Levi's",
    ...
    # ---- _vendor -----------------------------------------------------------
    "auth_jwt": ("Autentikasi JWT (OCA)", "Autentikasi bearer token JWT untuk API keluar-masuk."),
    "base_rest": ("Base REST (OCA)", "Kerangka membangun REST API tingkat tinggi di Odoo. Tidak dipasang."),
    "partner_firstname": (
        "Nama Depan/Belakang Partner (OCA)",
        "Memisahkan nama depan dan nama belakang untuk partner perorangan.",
    ),
    "queue_job": (
        "Antrean Pekerjaan (OCA)",
    ...
    "custom_pph_witholding": ("Mesin Pemotongan PPh", "Registry tarif, perhitungan, dan log penerapan pemotongan PPh."),
    "custom_pdp_audit": ("Audit Log UU PDP", "Log audit append-only berantai-hash, dilindungi trigger Postgres."),
    "custom_pdp_consent": (
        "Manajemen Consent",
        "Pencatatan pemberian dan penarikan persetujuan subjek data, teraudit.",
    ),
    "custom_pdp_core": ("Klasifikasi Data Pribadi", "Taksonomi klasifikasi data pribadi sesuai UU 27/2022."),
    "custom_pdp_dsar": ("Permintaan Subjek Data (DSAR)", "Alur permintaan akses/koreksi/penghapusan data pribadi."),
    "custom_pdp_masking": ("Masking PII", "Layanan penyamaran data pribadi dengan hook pada pembacaan ORM."),
    "custom_pdp_retention": ("Kebijakan Retensi Data", "Kebijakan retensi dan otomasi siklus hidup data pribadi."),
    # ---- control_plane -----------------------------------------------------
    "custom_hub_console": (
        "Konsol Hub Terpadu",
    ...
    "custom_core": ("Fondasi Platform", "Utilitas bersama, mixin, helper kebijakan, dan endpoint ber-HMAC."),
    "custom_currency_nbsp": (
        "Perbaikan Format Mata Uang & CSV",
        "Menghapus non-breaking space pada nominal dan menambahkan BOM UTF-8 pada ekspor CSV.",
    ),
    "custom_hht_bridge": (
        "Jembatan Handheld Terminal",
    ...
    "custom_product_barcode": (
        "Barcode Produk Ganda",
        "Beberapa barcode alternatif per varian produk — satu varian, satu stok, semua terpindai.",
    ),
    "custom_sale_bast": ("BAST dari Sales Order", "Membuat dan mengakses dokumen BAST langsung dari Sales Order."),
    # ---- ee_gap: keuangan --------------------------------------------------
    ...
    "custom_esg": ("Pelaporan ESG", "Metrik lingkungan, sosial, dan tata kelola untuk POJK 51/2017."),
    "custom_finance_budget": ("Anggaran Biaya Divisi", "Anggaran biaya per divisi/periode yang disinkronkan dari SAP."),
    ...
    "custom_payment_id": ("Payment Gateway Indonesia", "Adapter Midtrans, Xendit, dan DOKU untuk payment.provider."),
    "custom_payment_methods_id": (
        "Metode Pembayaran GIRO & Transfer",
        "Menambahkan GIRO dan BANK TRANSFER sebagai metode pembayaran pada jurnal bank.",
    ),
    "custom_payment_voucher": (
        "Voucher & Kuitansi Pembayaran",
    ...
    "custom_expenses": (
        "Klaim Biaya Karyawan",
        "Ekstraksi struk dengan OCR AI, persetujuan, kartu korporat, dan klaim kilometer.",
    ),
    "custom_fleet_id": ("Armada Kendaraan", "Pengingat STNK dan KIR, pencatatan BBM, dan penugasan pengemudi."),
    "custom_frontdesk": (
        "Front Desk & Tamu",
    ...
    "custom_hr_referral": ("Program Referral Karyawan", "Pelacakan kandidat rujukan beserta buku besar imbalannya."),
    "custom_hr_sso_keycloak": (
        "SSO Karyawan (Keycloak)",
    ...
    "custom_planning": ("Perencanaan Shift", "Perencanaan sumber daya dan penjadwalan shift tim."),
    ...
    "custom_pos_id": ("POS Indonesia", "QRIS, pembulatan rupiah, dan struk elektronik via WhatsApp/SMS."),
    "custom_operating_unit_pos": (
        "Unit Operasional di Kasir",
        "Membatasi POS, sesi, dan pesanan per toko, serta membubuhkan unit pada setiap baris jurnal penutupan sesi.",
    ),
    ...
    "custom_helpdesk": ("Helpdesk & SLA", "Alur tiket dengan SLA, eskalasi, dan portal pelanggan."),
    "custom_ops_reports": (
        "Laporan Operasional Armada",
        "Opname aset, pergerakan per event, suku cadang, kesehatan pemeliharaan, dan riwayat perbaikan.",
    ),
    "custom_project_api": ("API PMO", "Permukaan REST ber-JWT dan HMAC untuk aplikasi VAS PMO."),
    "custom_project_cr": (
        "Change Request Proyek",
    ...
    "custom_rental_quality_hook": (
        "Pemeriksaan Kualitas Sewa",
        "Quality check otomatis saat pengembalian, menautkan aset sewa ke equipment pemeliharaan.",
    ),
    "custom_timesheet": ("Timesheet & Penagihan Jasa", "Timesheet billable dengan integrasi lembur ke payroll."),
    # ---- ee_gap: manufaktur ------------------------------------------------
    "custom_iot_bridge": (
        "Jembatan IoT",
    ...
    "custom_mrp_plm": ("Manajemen Siklus Produk (PLM)", "Alur ECO, versi BoM, dan perubahan yang dikunci persetujuan."),
    "custom_quality_full": (
        "Manajemen Kualitas",
    ...
    "custom_dashboards": ("Dashboard KPI", "Dashboard berbasis ubin dengan kueri bahasa alami."),
    "custom_data_cleaning": (
        "Pembersihan Data",
        "Aturan deduplikasi dan normalisasi format Indonesia (nomor telepon, NIK).",
    ),
    "custom_documents": ("Manajemen Dokumen", "Workspace, penandaan, versi, dan akses yang sadar klasifikasi PDP."),
    "custom_knowledge": ("Basis Pengetahuan", "Wiki internal ringan dengan template dan versi artikel."),
    "custom_sign": ("Tanda Tangan Elektronik", "Alur tanda tangan multi-penandatangan dengan portal bertoken."),
    "custom_spreadsheet": (
        "Spreadsheet Terintegrasi",
        "Lapisan workbook dengan impor/ekspor CSV, bantuan AI, versi, dan berbagi.",
    ),
    "custom_studio_lite": ("Studio Lite", "Pengelola field kustom dan ekstensi tampilan secara deklaratif."),
    "custom_todo": ("Daftar Tugas Pribadi", "Timer pomodoro, pemecahan tugas oleh AI, dan template berulang."),
    # ---- ee_gap: pemasaran -------------------------------------------------
    "custom_affiliate": (
        "Program Afiliasi",
    ...
    "custom_forum": ("Forum Komunitas", "Moderasi AI, gamifikasi, dan penyamaran identitas penulis sesuai PDP."),
    "custom_livechat": (
        "Live Chat",
    ...
    "custom_social": ("Media Sosial", "Pengelolaan akun dan penjadwalan unggahan media sosial."),
    "custom_survey": (
        "Survei & NPS",
        "Pulse karyawan, NPS pelanggan, sertifikasi, dan survei terkait penilaian kinerja.",
    ),
    "custom_voip": ("Telepon & VoIP", "Click-to-call dan pencatatan panggilan dengan beberapa adapter SIP/PBX."),
    "custom_whatsapp": (
        "WhatsApp Bisnis",
    ...
    "custom_operating_unit_docs": (
        "Pembatasan Data per Unit Operasional",
