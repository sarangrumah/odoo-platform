...
add(
    {
        ...
        "rows": [
            [
                "ERP-as-a-Service Multi-Tenant",
                "Hub stack lengkap + orchestrator",
                "Internal tenants & afiliasi Erajaya",
            ],
            [
                "Localized Compliance Bundle",
                "custom_coretax* + Pajakku + custom_pdp_* + custom_hr_payroll_id",
                "UMKM ekosistem Erajaya (Eraspace partners, F&B franchisee)",
            ],
            [
                "AI Operations Layer",
                "ai-gateway + custom_ai_features + predictor",
                "Bundled tiap tenant; advanced tier opt-in",
            ],
            [
                "HHT / Field Ops Bridge",
                "custom_hht_bridge (Zebra PWA) + field_service",
                "Service Center, distribution, warehouse",
            ],
            [
                "Vertical Template Accelerator",
                "Repo modul + onboarding journey",
                "Vertikal baru di dalam Erajaya Group",
            ],
        ],
...
def build_pptx(out_path: Path, slides=None, footer_text: str = DEFAULT_FOOTER):
    ...
    def header_band(slide, title, subtitle=None):
        ...
        add_text(slide, Inches(0.5), Inches(0.18), SW - Inches(1), Inches(0.55), title, size=24, bold=True, color=WHITE)
        # Footer band
    ...
    for s in slides:
        ...
        if kind == "title":
            ...
            add_text(
                slide, Inches(1.2), Inches(3.6), SW - Inches(2.4), Inches(0.8), s["subtitle"], size=24, color=SLATE_2
            )
            add_rect(slide, Inches(1.2), Inches(4.5), Inches(1.8), Inches(0.05), BRAND)
            add_text(slide, Inches(1.2), Inches(5.0), SW - Inches(2.4), Inches(0.5), s["footer"], size=12, color=MUTED)
            continue
        ...
        if kind == "bullets":
            cur_top = content_top
            if s.get("intro"):
                add_text(slide, content_left, cur_top, content_w, Inches(0.5), s["intro"], size=14, color=INK)
                cur_top += Inches(0.55)
            # Bullets
        ...
        if kind == "table":
            cur_top = content_top
            if s.get("intro"):
                add_text(slide, content_left, cur_top, content_w, Inches(0.5), s["intro"], size=13, color=MUTED)
                cur_top += Inches(0.55)
        ...
        if kind == "layers":
            ...
            for i, (tit, desc) in enumerate(layers):
                ...
                shade = [SLATE, SLATE_2, SLATE_3, RGBColor(0x9C, 0xA3, 0xAF), RGBColor(0xD1, 0xD5, 0xDB)]
                bar_color = shade[i % len(shade)]
                add_rect(slide, content_left, y, Inches(0.18), row_h, bar_color)
                add_rect(slide, content_left + Inches(0.18), y, content_w - Inches(0.18), row_h, SURFACE)
                add_text(
                    slide,
        ...
        if kind == "modules_p1":
            ...
            if s.get("footnote"):
                add_text(
                    slide, content_left, SH - Inches(0.85), content_w, Inches(0.45), s["footnote"], size=10, color=MUTED
                )
            continue
        ...
        if kind == "two_col":
            ...
            for ci, (title_key, list_key, x_offset) in enumerate(
                ...
                add_rect(slide, x, cur_top + Inches(0.53), col_w, col_h - Inches(0.53), SURFACE)
                tb = slide.shapes.add_textbox(
                    x + Inches(0.2), cur_top + Inches(0.6), col_w - Inches(0.4), col_h - Inches(0.7)
                )
                tf = tb.text_frame
                tf.word_wrap = True
        ...
        if kind == "diagram":
            cur_top = content_top
            if s.get("intro"):
                add_text(slide, content_left, cur_top, content_w, Inches(0.4), s["intro"], size=13, color=MUTED)
                cur_top += Inches(0.45)
            ...
            tb = slide.shapes.add_textbox(
                x + Inches(0.2), cur_top + Inches(0.6), right_w - Inches(0.4), box_h - Inches(0.7)
            )
            tf = tb.text_frame
            tf.word_wrap = True
        ...
        if kind == "roadmap":
            ...
            for i, (qt, items) in enumerate(qs):
                ...
                add_rect(slide, x, content_top + Inches(0.63), col_w, avail_h - Inches(0.63), SURFACE)
                tb = slide.shapes.add_textbox(
                    x + Inches(0.15), content_top + Inches(0.7), col_w - Inches(0.3), avail_h - Inches(0.8)
                )
                tf = tb.text_frame
                tf.word_wrap = True
        ...
        if kind == "cta":
            cur_top = content_top
            if s.get("intro"):
                add_text(slide, content_left, cur_top, content_w, Inches(0.5), s["intro"], size=14, color=MUTED)
                cur_top += Inches(0.6)
            items = s["items"]
            ...
            for i, (num, ttl, desc) in enumerate(items):
                ...
                # Number circle
                circ = slide.shapes.add_shape(MSO_SHAPE.OVAL, content_left, y + Inches(0.05), Inches(0.7), Inches(0.7))
                circ.fill.solid()
...
def build_pdf(out_path: Path, slides=None, footer_text: str = None, doc_title: str = "Erajaya VAS — Odoo Platform"):
    ...
    sty_subtitle = ParagraphStyle(
        "subtitle", parent=styles["Normal"], fontName="Helvetica", fontSize=16, textColor=RL_SLATE_2, leading=22
    )
    sty_h = ParagraphStyle(
        "h", parent=styles["Heading1"], fontName="Helvetica-Bold", fontSize=18, textColor=rl_colors.white, leading=22
    )
    sty_body = ParagraphStyle(
        "body", parent=styles["Normal"], fontName="Helvetica", fontSize=11, textColor=RL_INK, leading=15
    )
    sty_muted = ParagraphStyle(
        "muted", parent=styles["Normal"], fontName="Helvetica-Oblique", fontSize=9, textColor=RL_MUTED, leading=12
    )
    sty_bullet = ParagraphStyle("bullet", parent=sty_body, leftIndent=14, bulletIndent=2, fontSize=11, leading=15)
    sty_col_h = ParagraphStyle(
        "colh", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=12, textColor=rl_colors.white, leading=14
    )
    sty_small = ParagraphStyle("small", parent=sty_body, fontSize=9, leading=11)
    sty_tile_val = ParagraphStyle(
        "tileval",
    ...
    sty_quarter_h = ParagraphStyle(
        "qh", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=11, textColor=rl_colors.white, leading=13
    )
    sty_layer_t = ParagraphStyle(
        "lt", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=12, textColor=RL_SLATE, leading=14
    )
    sty_layer_d = ParagraphStyle(
        "ld", parent=styles["Normal"], fontName="Helvetica", fontSize=9.5, textColor=RL_INK, leading=12
    )
    sty_code = ParagraphStyle(
        ...
        fontName="Courier",
        fontSize=9,
        textColor=rl_colors.HexColor("#FAFAF9"),
        leading=12,
    )
    ...
    def page_header_para(title):
        ...
        t = Table([[Paragraph(title, sty_h)]], colWidths=[PW - margin_l - margin_r], rowHeights=[0.9 * cm])
        t.setStyle(
            TableStyle(
                [
            ...
            body.append(
                [
                    Paragraph(
                        str(c),
                        ParagraphStyle(
                            "td", fontName="Helvetica", fontSize=font_size, textColor=RL_INK, leading=font_size + 2
                        ),
                    )
                    for c in r
                ]
    ...
    for s in rest:
        ...
        if kind == "bullets":
            ...
            if s.get("highlights"):
                ...
                tiles = s["highlights"]
                tile_data = [
                    [Paragraph(v, sty_tile_val) for v, _ in tiles],
                    [Paragraph(l, sty_tile_lbl) for _, l in tiles],
                ]
                w_each = (avail_w - 0.6 * cm) / len(tiles)
                ttbl = Table(tile_data, colWidths=[w_each] * len(tiles), rowHeights=[1.3 * cm, 0.7 * cm])
                ts = [
            ...
            flowables.append(std_table(s["headers"], s["rows"], col_widths=s.get("col_widths"), font_size=9))
            if s.get("footnote"):
                flowables.append(Spacer(1, 0.25 * cm))
            ...
            for i, (tit, desc) in enumerate(s["layers"]):
                ...
                tbl = Table(
                    [["", Paragraph(f"<b>{tit}</b><br/><font size=9 color='#111827'>{desc}</font>", sty_layer_t)]],
            ...
            st = s["summary_table"]
            flowables.append(std_table(st["headers"], st["rows"], col_widths=st["col_widths"], font_size=10))
            if s.get("footnote"):
                flowables.append(Spacer(1, 0.25 * cm))
            ...
            for gt, desc in groups:
                inner = Table(
                    [
                        [
                            Paragraph(
                                f"<b><font color='#1F2937'>{gt}</font></b><br/><font size=9>{desc}</font>", sty_layer_t
                            )
                        ]
                    ],
            ...
            ascii_text = "<br/>".join(line.replace(" ", "&nbsp;") for line in s["ascii"])
            ...
            grid = Table([[left_cell, right_cell]], colWidths=[left_w, right_w])
            grid.setStyle(
                TableStyle(
                    [
                        ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ...
            for num, ttl, desc in s["items"]:
                num_cell = Paragraph(
                    f"<para align='center'><font color='white' size=16><b>{num}</b></font></para>", sty_body
                )
                txt_cell = Paragraph(
                    f"<font color='#1F2937' size=13><b>{ttl}</b></font><br/><font size=10>{desc}</font>", sty_body
                )
                row = Table([[num_cell, txt_cell]], colWidths=[1.5 * cm, avail_w - 1.5 * cm])
                row.setStyle(
                    TableStyle(
                        [
            ...
            flowables.append(
                Paragraph(
                    f"<para align='center'><font size=40 color='#1F2937'><b>{s['title']}</b></font></para>", sty_body
                )
            )
            flowables.append(Spacer(1, 0.4 * cm))
            ...
            flowables.append(Spacer(1, 0.5 * cm))
            flowables.append(
                Paragraph(f"<para align='center'><font size=18 color='#374151'>{s['subtitle']}</font></para>", sty_body)
            )
            flowables.append(Spacer(1, 2 * cm))
            flowables.append(
                Paragraph(f"<para align='center'><font size=10 color='#6B7280'>{s['footer']}</font></para>", sty_body)
            )
