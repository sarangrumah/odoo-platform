...
BRANDS = ["PT Bank", "BCA", "BRI", "BNI", "Mandiri", "CIMB", "Permata", "Danamon",
          "Maybank", "OCBC", "Panin", "HSBC", "Citibank", "Time Deposit", "Time Loan"]
...
REQUIRED = ["1117200001", "2104300001", "2104100004", "2104100005",
            "1106000001", "2103100001", "5199000000", "6199000000",
            "7607000000", "7704000000"]
...
def main():
    ...
    write_csv("account.account-erajaya.csv",
              ["id", "code", "name", "account_type", "reconcile"], acc_rows)
    ...
    NAMES1 = {"1": "Aset", "2": "Kewajiban", "3": "Ekuitas", "4": "Pendapatan",
              "5": "Pendapatan", "6": "Beban Pokok", "7": "Beban", "8": "Lain-lain"}
    ...
    for p in pref2:
        grp_rows.append(["erajaya_group_%s" % p, p, p,
                         "%s %s" % (NAMES1.get(p[0], "Group"), p)])
    write_csv("account.group-erajaya.csv",
              ["id", "code_prefix_start", "code_prefix_end", "name"], grp_rows)
    ...
    write_csv("account.tax.group-erajaya.csv",
              ["id", "country_id", "name", "tax_payable_account_id",
               "tax_receivable_account_id"], tg_rows)

    TAX_HDR = ["id", "active", "description", "is_base_affected", "invoice_label",
               "type_tax_use", "tax_group_id", "name", "amount_type", "amount",
               "repartition_line_ids/repartition_type", "repartition_line_ids/tag_ids",
               "repartition_line_ids/factor_percent",
               "repartition_line_ids/document_type", "repartition_line_ids/account_id"]
    ...
    for t in taxes:
        ...
        rep = t["repartition"]
        r0 = rep[0]
        tax_rows.append([n, "", t["description"] or "",
                         "" if t["is_base_affected"] else "False",
                         t["invoice_label"] or "", t["type_tax_use"], gxid, t["name"],
                         t["amount_type"], str(t["amount"]),
                         r0["rep_type"], "", fact(r0["factor"]), r0["doc_type"],
                         acct(r0["account_code"])])
        for r in rep[1:]:
            tax_rows.append(["", "", "", "", "", "", "", "", "", "",
                             r["rep_type"], "", fact(r["factor"]), r["doc_type"],
                             acct(r["account_code"])])
